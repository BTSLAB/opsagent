---
description: Update OpsAgent from upstream when branch has diverged (ahead/behind)
---

# OpsAgent Upstream Sync Workflow
...
```bash
# Kill running app
pkill -x "OpsAgent" || true

# Move old version
mv /Applications/OpsAgent.app /tmp/OpsAgent-backup.app

# Install new build
cp -R dist/OpsAgent.app /Applications/

# Launch
open /Applications/OpsAgent.app
```
...
pnpm opsagent health

# Verify no zombie processes
ps aux | grep -E "(opsagent|gateway)" | grep -v grep

# Test agent functionality by sending a verification message
pnpm opsagent agent --message "Verification: macOS app rebuild successful - agent is responding." --session-id YOUR_TELEGRAM_SESSION_ID
...
# Update opsagent.json with fallback chains
...
pnpm opsagent agent --message "Verification: Upstream sync and macOS rebuild completed successfully." --session-id YOUR_TELEGRAM_SESSION_ID || echo "Warning: Agent test failed - check Telegram for verification message"
