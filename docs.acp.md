...
This document describes how the OpsAgent ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`opsagent acp` exposes an ACP agent over stdio and forwards prompts to a running
OpsAgent Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
drive a OpsAgent Gateway session.

Quick steps:

1. Run a Gateway (local or remote).
2. Configure the Gateway target (`gateway.remote.url` + auth) or pass flags.
3. Point the IDE to run `opsagent acp` over stdio.

Example config:

```bash
opsagent config set gateway.remote.url wss://gateway-host:18789
opsagent config set gateway.remote.token <token>
```

Example run:

```bash
opsagent acp --url wss://gateway-host:18789 --token <token>
```
...
```bash
opsagent acp --session agent:main:main
opsagent acp --session agent:design:main
opsagent acp --session agent:qa:bug-123
```
...
In Zed, open the Agent panel and select “OpsAgent ACP” to start a thread.

## Execution Model

- ACP client spawns `opsagent acp` and speaks ACP messages over stdio.
...
```bash
opsagent acp --session agent:main:main
opsagent acp --session-label "support inbox"
opsagent acp --reset-session
```
...
`opsagent acp` resolves the Gateway URL and auth from CLI flags or config:
