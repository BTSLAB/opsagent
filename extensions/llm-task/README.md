...
`clawd.invoke --each`) without adding new OpsAgent code per workflow.
...
This extension depends on OpsAgent internal modules (the embedded agent runner).
It is intended to ship as a **bundled** OpsAgent extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.opsagent/extensions` as a standalone plugin directory.
