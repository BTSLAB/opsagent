...
// NOTE: This extension is intended to be bundled with OpsAgent.
// When running from source (tests/dev), OpsAgent internals live under src/.
...
export function createLlmTaskTool(api: OpsAgentPluginApi) {
  return {
    name: "llm-task",
    description:
    ...
    async execute(_id: string, params: Record<string, unknown>) {
      ...
      try {
        tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "opsagent-llm-task-"));
