...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.OPSAGENT_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "opsagent-nostr-"));
  process.env.OPSAGENT_STATE_DIR = dir;
  setNostrRuntime({
    state: {
  ...
  try {
    ...
    if (previous === undefined) delete process.env.OPSAGENT_STATE_DIR;
    else process.env.OPSAGENT_STATE_DIR = previous;
