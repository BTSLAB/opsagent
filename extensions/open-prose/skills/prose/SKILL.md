...
metadata: {"opsagent":{"emoji":"🪶","homepage":"https://www.prose.md"}}
---
...
## OpsAgent Runtime Mapping

- **Task tool** in the upstream spec == OpsAgent `sessions_spawn`
- **File I/O** == OpsAgent `read`/`write`
- **Remote fetch** == OpsAgent `web_fetch` (or `exec` with curl when POST is required)
