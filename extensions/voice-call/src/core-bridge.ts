...
function resolveOpsAgentRoot(): string {
  if (coreRootCache) return coreRootCache;
  const override = process.env.OPSAGENT_ROOT?.trim();
  if (override) {
  ...
  for (const start of candidates) {
    const found = findPackageRoot(start, "opsagent");
    if (found) {
      coreRootCache = found;
      return found;
    }
  ...
  throw new Error(
    "Unable to resolve OpsAgent root. Set OPSAGENT_ROOT to the package root.",
  );
}

async function importCoreModule<T>(relativePath: string): Promise<T> {
  const root = resolveOpsAgentRoot();
