...
OpsAgent extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
opsagent channels login --channel zalouser
...
```bash
opsagent message send --channel zalouser --target <threadId> --message "Hello from OpsAgent!"
```
...
```bash
opsagent channels login --channel zalouser              # Login via QR
opsagent channels login --channel zalouser --account work
opsagent channels status --probe
opsagent channels logout --channel zalouser
```

### Directory (IDs, contacts, groups)

```bash
opsagent directory self --channel zalouser
opsagent directory peers list --channel zalouser --query "name"
opsagent directory groups list --channel zalouser --query "work"
opsagent directory groups members --channel zalouser --group-id <id>
```
...
```bash
# Text
opsagent message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
opsagent message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `opsagent channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
opsagent channels login --channel zalouser --account work
opsagent message send --channel zalouser --account work --target <id> --message "Hello"
