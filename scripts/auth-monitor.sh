...
#
# Suggested cron: */30 * * * * /home/admin/opsagent/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send OpsAgent notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., opsagent-alerts)
...
STATE_FILE="$HOME/.opsagent/auth-monitor-state"
...
send_notification() {
    ...
    # Send via OpsAgent if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use opsagent
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via OpsAgent to $NOTIFY_PHONE..."
            opsagent send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! OpsAgent is down. Run: ssh l36 '~/opsagent/scripts/mobile-reauth.sh'" "urgent"
    exit 1
