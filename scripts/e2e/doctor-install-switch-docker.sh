...
IMAGE_NAME="opsagent-doctor-install-switch-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  export PATH="/tmp/opsagent-bin:$PATH"
  mkdir -p /tmp/opsagent-bin

  cat > /tmp/opsagent-bin/systemctl <<"SYSTEMCTL"
#!/usr/bin/env bash
set -euo pipefail
...
SYSTEMCTL
  chmod +x /tmp/opsagent-bin/systemctl

  cat > /tmp/opsagent-bin/loginctl <<"LOGINCTL"
#!/usr/bin/env bash
set -euo pipefail
...
exit 0
LOGINCTL
  chmod +x /tmp/opsagent-bin/loginctl
  ...
  npm_bin="/tmp/npm-prefix/bin/opsagent"
  npm_entry="/tmp/npm-prefix/lib/node_modules/opsagent/dist/entry.js"
  ...
  run_flow() {
    ...
    echo "== Flow: $name =="
    home_dir=$(mktemp -d "/tmp/opsagent-switch-${name}.XXXXXX")
    export HOME="$home_dir"
    export USER="testuser"

    eval "$install_cmd"

    unit_path="$HOME/.config/systemd/user/opsagent-gateway.service"
    if [ ! -f "$unit_path" ]; then
