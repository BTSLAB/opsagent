...
IMAGE_NAME="opsagent-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="opsagent-net-e2e-$$"
GW_NAME="opsagent-gateway-e2e-$$"
...
echo "Starting gateway container..."
	docker run --rm -d \
	  --name "$GW_NAME" \
	  --network "$NET_NAME" \
	  -e "OPSAGENT_GATEWAY_TOKEN=$TOKEN" \
	  -e "OPSAGENT_SKIP_CHANNELS=1" \
	  -e "OPSAGENT_SKIP_GMAIL_WATCHER=1" \
	  -e "OPSAGENT_SKIP_CRON=1" \
	  -e "OPSAGENT_SKIP_CANVAS_HOST=1" \
	  "$IMAGE_NAME" \
