...
IMAGE_NAME="opsagent-onboard-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  cat > /tmp/opsagent-bin/trash <<'"'"'TRASH'"'"'
#!/usr/bin/env bash
set -euo pipefail
...
TRASH
  chmod +x /tmp/opsagent-bin/trash
  ...
  run_wizard_cmd() {
    ...
    input_fifo="$(mktemp -u "/tmp/opsagent-onboard-${case_name}.XXXXXX")"
    mkfifo "$input_fifo"
    local log_path="/tmp/opsagent-onboard-${case_name}.log"
  ...
  make_home() {
    mktemp -d "/tmp/opsagent-e2e-$1.XXXXXX"
  }
  ...
  run_case_local_basic() {
    ...
    config_path="$HOME/.opsagent/opsagent.json"
    sessions_dir="$HOME/.opsagent/agents/main/sessions"
...
NODE
  ...
  run_case_reset() {
    ...
    cat > "$HOME/.opsagent/opsagent.json" <<'"'"'JSON'"'"'
{
