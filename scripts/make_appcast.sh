...
ZIP=${1:?"Usage: $0 OpsAgent-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/opsagent/opsagent/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^OpsAgent-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/opsagent/opsagent/releases/download/v${VERSION}/"}
