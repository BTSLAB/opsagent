...
# Build and bundle OpsAgent into a minimal .app we can open.
# Outputs to dist/OpsAgent.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/OpsAgent.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="OpsAgent"
BUNDLE_ID="${BUNDLE_ID:-com.opsagent.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/opsagent/opsagent/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/OpsAgent/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :OpsAgentBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :OpsAgentGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/OpsAgent"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/OpsAgent"
fi
chmod +x "$APP_ROOT/Contents/MacOS/OpsAgent"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/OpsAgent" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/OpsAgent/Resources/OpsAgent.icns" "$APP_ROOT/Contents/Resources/OpsAgent.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/OpsAgent/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying OpsAgentKit resources"
OPSAGENTKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/OpsAgentKit_OpsAgentKit.bundle"
if [ -d "$OPSAGENTKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/OpsAgentKit_OpsAgentKit.bundle"
  cp -R "$OPSAGENTKIT_BUNDLE" "$APP_ROOT/Contents/Resources/OpsAgentKit_OpsAgentKit.bundle"
else
  echo "WARN: OpsAgentKit resource bundle not found at $OPSAGENTKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running OpsAgent"
killall -q OpsAgent 2>/dev/null || true
