...
BASE_IMAGE="${BASE_IMAGE:-opsagent-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-opsagent-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=opsagent.sandbox=1)
NOTE
