#!/bin/bash
# Setup OpsAgent Auth Management System
...
echo "=== OpsAgent Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'opsagent-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  OpsAgent message: Send warning via OpsAgent itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/opsagent-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/opsagent-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/opsagent-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now opsagent-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status opsagent-auth-monitor.timer"
echo ""
echo "Quick commands:"
