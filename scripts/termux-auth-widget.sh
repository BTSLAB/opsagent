...
# OpsAgent Auth Widget for Termux
...
SERVER="${OPSAGENT_SERVER:-l36}"

# Check auth status
termux-toast "Checking OpsAgent auth..."

STATUS=$(ssh "$SERVER" '$HOME/opsagent/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/opsagent/scripts/claude-auth-status.sh json' 2>&1)
    ...
    CLAUDE_EXPIRING|OPSAGENT_EXPIRING)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/opsagent/scripts/mobile-reauth.sh'"
                ;;
            *)
    ...
    CLAUDE_EXPIRED|OPSAGENT_EXPIRED)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: OpsAgent won't work until re-auth"
                ;;
        esac
        ;;
