...
if [ -z "$doc_path" ]; then
    ...
    echo "The opsagent docs require JavaScript rendering."
    echo ""
    echo "Available docs (first 20):"
    "$SCRIPT_DIR/cache.sh" urls | sed 's|https://docs.opsagent.dev/||' | head -20
...
doc_path="${doc_path#https://docs.opsagent.dev/}"

url="https://docs.opsagent.dev/${doc_path}"
