#!/bin/bash
# Fetch and display OpsAgent documentation sitemap organized by category

SITEMAP_URL="https://docs.opsagent.dev/sitemap.xml"

echo "📚 OpsAgent Documentation Structure"
echo "===================================="
echo ""
...
print_category() {
    ...
    local matches=$(echo "$URLS" | grep -E "$pattern" | sed 's|https://docs.opsagent.dev/||')
    if [ -n "$matches" ]; then
        echo "$emoji $category"
        echo "$matches" | sed 's/^/  - /'
        echo ""
