# OpsAgent Security Check

🔒 **Self-security audit framework for OpsAgent**

Inspired by the security hardening framework from [ᴅᴀɴɪᴇʟ ᴍɪᴇssʟᴇʀ](https://x.com/DanielMiessler) and integrated with [official ClawdBot security documentation](https://docs.opsagent.dev/gateway/security).

This skill teaches OpsAgent to audit its own security posture using first-principles reasoning. Not a hard-coded script—it's a **knowledge framework** that OpsAgent applies dynamically to detect vulnerabilities, understand their impact, and recommend specific remediations.

## What This Is

- 🧠 **Knowledge-based** - Embeds the security framework directly in OpsAgent
- 🔍 **Dynamic detection** - OpsAgent learns to find issues, not just run a script
...
```bash
# Via ClawdHub
clawdhub install opsagent-security-check

# Or clone for manual installation
git clone https://github.com/TheSethRose/OpsAgent-Security-Check.git
cp -r OpsAgent-Security-Check ~/.opsagent/skills/
```
...
### Via OpsAgent
```
@opsagent audit my security
@opsagent run security check
@opsagent what vulnerabilities do I have?
@opsagent security audit --deep
@opsagent security audit --fix
```
...
- Tightens permissions: `.opsagent` to `700`, configs to `600`
...
├── skill.json    # OpsAgent metadata
├── README.md     # This file
└── .gitignore
```

**SKILL.md** is the source of truth—it teaches OpsAgent everything it needs to know.
...
- ✅ OpsAgent understands the "why" behind each check
...
**OpsAgent knows its attack surface. Do you?**
