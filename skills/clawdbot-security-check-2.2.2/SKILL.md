---
name: opsagent-self-security-audit
description: Perform a comprehensive read-only security audit of OpsAgent's own configuration. This is a knowledge-based skill that teaches OpsAgent to identify hardening opportunities across the system. Use when user asks to "run security check", "audit opsagent", "check security hardening", or "what vulnerabilities does my OpsAgent have". This skill uses OpsAgent's internal capabilities and file system access to inspect configuration, detect misconfigurations, and recommend remediations. It is designed to be extensible - new checks can be added by updating this skill's knowledge.
homepage: https://github.com/TheSethRose/OpsAgent-Security-Check
metadata: {"opsagent":{"emoji":"🔒","os":["darwin","linux"],"requires":{"files":["read"],"tools":["exec","bash"]},"install":[{"id":"read-skill","kind":"skill","name":"opsagent-self-security-audit","label":"Install security audit skill","bins":["SKILL.md"]}]}}
---

# OpsAgent Self-Security Audit Framework

This skill empowers OpsAgent to audit its own security posture using first-principles reasoning. Rather than relying on a static script, OpsAgent learns the framework and applies it dynamically to detect vulnerabilities, understand their impact, and recommend specific remediations.

## Core Philosophy

> "Security through transparency and self-awareness." — Inspired by ᴅᴀɴɪᴇʟ ᴍɪᴇssʟᴇʀ

OpsAgent should know its own attack surface. This skill embeds that knowledge directly.
...
- `opsagent security audit` — Standard audit of common issues
- `opsagent security audit --deep` — Comprehensive audit with all checks
- `opsagent security audit --fix` — Apply guardrail remediations

## The 12 Security Domains

When auditing OpsAgent, systematically evaluate these domains:

### 1. Gateway Exposure 🔴 Critical

**What to check:**
- Where is the gateway binding? (`gateway.bind`)
- Is authentication configured? (`gateway.auth_token` or `OPSAGENT_GATEWAY_TOKEN` env var)
- What port is exposed? (default: 18789)
- Is WebSocket auth enabled?

**How to detect:**
```bash
cat ~/.opsagent/opsagent.json | grep -A10 '"gateway"'
env | grep OPSAGENT_GATEWAY_TOKEN
```
...
opsagent doctor --generate-gateway-token
export OPSAGENT_GATEWAY_TOKEN="$(openssl rand -hex 32)"
```
...
**How to detect:**
```bash
cat ~/.opsagent/opsagent.json | grep -E '"dm_policy|"allowFrom"'
```

**Vulnerability:** Setting to `allow` or `open` means any user can DM OpsAgent.
...
**How to detect:**
```bash
cat ~/.opsagent/opsagent.json | grep -E '"groupPolicy"|"groups"' 
cat ~/.opsagent/opsagent.json | grep -i "mention"
```
...
| Platform | Path |
|----------|------|
| WhatsApp | `~/.opsagent/credentials/whatsapp/{accountId}/creds.json` |
| Telegram | `~/.opsagent/opsagent.json` or env |
| Discord | `~/.opsagent/opsagent.json` or env |
| Slack | `~/.opsagent/opsagent.json` or env |
| Pairing allowlists | `~/.opsagent/credentials/channel-allowFrom.json` |
| Auth profiles | `~/.opsagent/agents/{agentId}/auth-profiles.json` |
| Legacy OAuth | `~/.opsagent/credentials/oauth.json` |

**How to detect:**
```bash
ls -la ~/.opsagent/credentials/
ls -la ~/.opsagent/agents/*/auth-profiles.json 2>/dev/null
stat -c "%a" ~/.opsagent/credentials/oauth.json 2>/dev/null
```
...
```bash
chmod 700 ~/.opsagent
chmod 600 ~/.opsagent/credentials/oauth.json
chmod 600 ~/.opsagent/opsagent.json
```
...
**How to detect:**
```bash
cat ~/.opsagent/opsagent.json | grep -A5 '"browser"'
cat ~/.opsagent/opsagent.json | grep -i "controlUi|insecureAuth"
ls -la ~/.opsagent/browser/
```
...
```bash
cat ~/.opsagent/opsagent.json | grep -A10 '"gateway"'
cat ~/.opsagent/opsagent.json | grep '"tailscale"'
```
...
**How to detect:**
```bash
cat ~/.opsagent/opsagent.json | grep -i "restrict|mcp|elevated"
cat ~/.opsagent/opsagent.json | grep -i "workspaceAccess|sandbox"
cat ~/.opsagent/opsagent.json | grep -i "openRoom"
```
...
chmod 700 ~/.opsagent
chmod 600 ~/.opsagent/opsagent.json
chmod 600 ~/.opsagent/credentials/*
```
...
**How to detect:**
```bash
cat ~/.opsagent/opsagent.json | grep -i "plugin|allowlist"
cat ~/.opsagent/opsagent.json | grep -i "model|anthropic"
```
...
**How to detect:**
```bash
cat ~/.opsagent/opsagent.json | grep -i "logging|redact"
ls -la ~/.opsagent/logs/
```
...
**How to detect:**
```bash
cat ~/.opsagent/opsagent.json | grep -i "untrusted|wrap"
```
...
**How to detect:**
```bash
cat ~/.opsagent/opsagent.json | grep -A10 '"blocked_commands"'
```
...
- Tightens local permissions: `.opsagent` directory to `700`, config files to `600`
...
### Containment
1. **Stop the gateway process** — `opsagent daemon stop`
2. **Set gateway.bind to loopback** — `"bind": "127.0.0.1"`
3. **Disable risky DMs and groups** — Set to `disabled`

### Rotation
1. **Change the gateway auth token** — `opsagent doctor --generate-gateway-token`
2. **Rotate browser control and hook tokens**
3. **Revoke and rotate API keys** for model providers

### Review
1. **Check gateway logs and session transcripts** — `~/.opsagent/logs/`
2. **Review recent config changes** — Git history or backups
3. **Re-run the security audit with the deep flag** — `opsagent security audit --deep`

## Reporting Vulnerabilities

Report security issues to: **security@opsagent.dev**
...
```bash
CONFIG_PATHS=(
  "$HOME/.opsagent/opsagent.json"
  "$HOME/.opsagent/config.yaml"
  "$HOME/.opsagent/.opsagentrc"
  ".opsagentrc"
)
...
This audit was performed by OpsAgent's self-security framework.
...
1. **Exposure:** What network interfaces can reach OpsAgent?
2. **Authentication:** What verification does each access point require?
3. **Isolation:** What boundaries exist between OpsAgent and the host?
4. **Trust:** What content sources are considered "trusted"?
5. **Auditability:** What evidence exists of OpsAgent's actions?
6. **Least Privilege:** Does OpsAgent have only necessary permissions?
...
- Official docs: https://docs.opsagent.dev/gateway/security
- Original framework: [ᴅᴀɴɪᴇʟ ᴍɪᴇssʟᴇʀ on X](https://x.com/DanielMiessler/status/2015865548714975475)
- Repository: https://github.com/TheSethRose/OpsAgent-Security-Check
- Report vulnerabilities: security@opsagent.dev

---

**Remember:** This skill exists to make OpsAgent self-aware of its security posture. Use it regularly, extend it as needed, and never skip the audit.
