...
```bash
~/.opsagent/skills/security/security stats
```

## Configuration

Edit `~/.opsagent/hooks/security-validator/config.json` to customize:
...
Works seamlessly with the Security Skill (`~/.opsagent/skills/security`) for:
...
All security events are logged to `~/.opsagent/logs/security-events.log`:
...
🐦 **Follow [@LexpertAI](https://x.com/LexpertAI)** for real-time security research, threat analysis, and updates to the OpsAgent Security Suite.
