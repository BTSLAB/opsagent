...
metadata: {"opsagent":{"emoji":"🏠","requires":{"bins":["jq","curl"]}}}
---
...
3. Click "Create Token", name it (e.g., "OpsAgent")
...
# In HA automation
action:
  - service: rest_command.notify_opsagent
    data:
...
rest_command:
  notify_opsagent:
    url: "https://your-opsagent-url/webhook/home-assistant"
    method: POST
    headers:
...
### 3. Handle in OpsAgent

OpsAgent receives the webhook and can notify you or take action based on the event.
