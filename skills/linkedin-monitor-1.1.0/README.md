...
![OpsAgent](https://img.shields.io/badge/opsagent-skill-orange.svg)
...
- [OpsAgent](https://github.com/opsagent/opsagent) installed and running
- A browser profile logged into LinkedIn
- A channel for alerts (Discord, Telegram, Slack, WhatsApp, or any OpsAgent channel)
...
LinkedIn Monitor uses OpsAgent's browser tool to check your inbox. You need a browser profile that's logged into LinkedIn.

**Option A: Use the default "clawd" profile**
```bash
# Start the clawd browser
opsagent browser start --profile clawd
...
opsagent browser start --profile linkedin
...
Configuration is saved to `~/.opsagent/linkedin-monitor/config.json`
...
**State is tracked in:** `~/.opsagent/linkedin-monitor/state/messages.json`
...
Edit `~/.opsagent/linkedin-monitor/config.json`:
...
Drafts are generated using your communication style from `USER.md` in your OpsAgent workspace.
...
rm -rf ~/.opsagent/linkedin-monitor
```
...
Part of the [OpsAgent](https://opsagent.dev) ecosystem.
