...
# Uses OpsAgent browser tool instead of lk CLI

set -e

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
CONFIG_DIR="${HOME}/.opsagent/linkedin-monitor"
...
# The actual browser check is done by OpsAgent
# This script is called by the cron, which then invokes OpsAgent
# to do the browser work and parse results

# Output instruction for OpsAgent cron
cat << 'EOF'
LINKEDIN_BROWSER_CHECK
Instructions for OpsAgent:
...
4. Compare against state file: ~/.opsagent/linkedin-monitor/state/messages.json
