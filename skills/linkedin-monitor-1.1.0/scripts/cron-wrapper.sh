...
# Runs the check and sends results to OpsAgent if new messages found
...
RESULT=$(bash "${SCRIPT_DIR}/check.sh" 2>&1) || {
    ...
    if echo "$RESULT" | jq -e '.error' &>/dev/null; then
        ...
        if [ -n "${CHANNEL_ID}" ]; then
            # Send error alert to OpsAgent
            # This uses the OpsAgent message format
            cat << EOF
...
# Format the messages for OpsAgent
...
# Output for OpsAgent cron to pick up
