#!/bin/bash
# OpsAgent Migration Import Script
...
# Defaults
WORKSPACE="${OPSAGENT_WORKSPACE:-$HOME/clawd}"
CONFIG_DIR="$HOME/.opsagent"
ARCHIVE=""
FORCE=false
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --help|-h)
            ...
            echo ""
            echo "Arguments:"
            echo "  ARCHIVE                Path to opsagent export archive"
            echo ""
            echo "Options:"
...
echo -e "${GREEN}🚀 OpsAgent Import${NC}"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
...
if [ -d "$STAGING/config" ]; then
    ...
    # Main config
    if [ -f "$STAGING/config/opsagent.json" ]; then
        cp "$STAGING/config/opsagent.json" "$CONFIG_DIR/"
...
if [ -f "$CONFIG_DIR/opsagent.json" ]; then
    # Check if jq is available
    if command -v jq &> /dev/null; then
        CURRENT_WS=$(jq -r '.agent.workspace // empty' "$CONFIG_DIR/opsagent.json" 2>/dev/null || true)
        if [ -n "$CURRENT_WS" ] && [ "$CURRENT_WS" != "$WORKSPACE" ]; then
            echo -e "${YELLOW}→ Updating workspace path in config...${NC}"
            jq --arg ws "$WORKSPACE" '.agent.workspace = $ws' "$CONFIG_DIR/opsagent.json" > "$CONFIG_DIR/opsagent.json.tmp"
            mv "$CONFIG_DIR/opsagent.json.tmp" "$CONFIG_DIR/opsagent.json"
        fi
    fi
fi
...
echo ""
echo "Next steps:"
echo "  1. Install OpsAgent (if not installed):"
echo "     npm install -g opsagent"
echo ""
echo "  2. Start the gateway:"
echo "     cd $WORKSPACE && opsagent gateway start"
echo ""
if [ -d "$STAGING/config/whatsapp" ]; then
    echo -e "  ${GREEN}✓${NC} WhatsApp session included - no re-scan needed!"
else
    echo "  3. Link WhatsApp (if needed):"
    echo "     opsagent whatsapp link"
fi
