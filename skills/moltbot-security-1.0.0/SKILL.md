...
description: Security hardening guide for Moltbot/OpsAgent. Lock down your gateway, fix file permissions, set up auth, configure firewalls. Based on real vulnerability research.
version: 1.0.0
author: NextFrontierBuilds
keywords: moltbot, opsagent, security, hardening, gateway, firewall, tailscale, ssh, authentication, ai-agent
---
...
**Based on:** Real vulnerability research that found 1,673+ exposed OpsAgent/Moltbot gateways on Shodan.
...
```bash
opsagent security audit --deep --fix
```
...
Check your `~/.opsagent/opsagent.json`:
...
```bash
export OPSAGENT_GATEWAY_TOKEN="your-secure-random-token-here"
```
...
```bash
export OPSAGENT_GATEWAY_PASSWORD="your-secure-password-here"
```
...
chmod 700 ~/.opsagent/credentials
```
...
**What this does:** Stops OpsAgent from announcing itself via mDNS/Bonjour.

Add to your shell config (`~/.zshrc` or `~/.bashrc`):

```bash
export OPSAGENT_DISABLE_BONJOUR=1
```
...
**What this does:** Creates encrypted tunnel between your devices. Access OpsAgent from anywhere without public exposure.
...
**Configure OpsAgent for Tailscale:**
