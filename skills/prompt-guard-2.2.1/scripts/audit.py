#!/usr/bin/env python3
"""
Security Audit Script for OpsAgent
...
class SecurityAudit:
    ...
    def check_config_not_in_sync(self) -> bool:
        ...
        opsagent_path = Path('~/.opsagent').expanduser().resolve()
        ...
        for sync_folder in sync_folders:
            if sync_folder.exists():
                try:
                    if opsagent_path.is_relative_to(sync_folder):
                        self.issues.append({
                            'check': 'Config not in cloud sync',
                            'status': 'fail',
                            'message': f'~/.opsagent is inside {sync_folder}!',
                            'fix': 'Move ~/.opsagent outside of cloud sync folders'
                        })
                        return False
                except ValueError:
    ...
    def check_gateway_config(self) -> bool:
        ...
        config_path = Path('~/.opsagent/opsagent.json').expanduser()
        if not config_path.exists():
            self.warnings.append({
                'check': 'Gateway config',
                'status': 'skip',
                'message': 'opsagent.json not found'
            })
            return True
    ...
    def run_audit(self, quick: bool = False) -> Tuple[int, int, int]:
        ...
        # File permissions
        self.check_dir_permissions('~/.opsagent', 0o700, 'OpsAgent directory permissions')
        self.check_file_permissions('~/.opsagent/opsagent.json', 0o600, 'Config file permissions')
    ...
    def print_report(self, verbose: bool = False):
        """Print audit report."""
        print("=" * 60)
        print("🛡️  OPSAGENT SECURITY AUDIT")
        print("=" * 60)
...
def main():
    parser = argparse.ArgumentParser(description='OpsAgent Security Audit')
