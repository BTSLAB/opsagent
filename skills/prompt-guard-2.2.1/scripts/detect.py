...
CRITICAL_PATTERNS = [
    ...
    r"(show|print|display|output|reveal|give|read|cat|type)\s*.{0,20}(config|\.env|opsagent\.json|credential)",
    ...
    r"cat\s+.{0,40}(\.env|config\.json|secret|credential|opsagent)",
...
SECRET_PATTERNS = {
    'en': [
        ...
        r"(read|cat|open|display)\s*.{0,30}(config|\.env|credential|opsagent\.json)",
