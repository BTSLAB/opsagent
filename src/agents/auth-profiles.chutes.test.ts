...
describe("auth-profiles (chutes)", () => {
  ...
  afterEach(async () => {
    ...
    if (previousAgentDir === undefined) delete process.env.OPSAGENT_AGENT_DIR;
    else process.env.OPSAGENT_AGENT_DIR = previousAgentDir;
  ...
  it("refreshes expired Chutes OAuth credentials", async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "opsagent-chutes-"));
    process.env.OPSAGENT_STATE_DIR = tempDir;
    process.env.OPSAGENT_AGENT_DIR = path.join(tempDir, "agents", "main", "agent");
