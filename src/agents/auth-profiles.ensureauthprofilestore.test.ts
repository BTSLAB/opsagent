...
describe("ensureAuthProfileStore", () => {
  it("migrates legacy auth.json and deletes it (PR #368)", () => {
    const agentDir = fs.mkdtempSync(path.join(os.tmpdir(), "opsagent-auth-profiles-"));
    try {
  ...
  it("merges main auth profiles into agent store and keeps agent overrides", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "opsagent-auth-merge-"));
    ...
    try {
      ...
      process.env.OPSAGENT_AGENT_DIR = mainDir;
  ...
  it("drops codex-cli from merged store when a custom openai-codex profile matches", async () => {
    await withTempHome(async (tempHome) => {
      const root = fs.mkdtempSync(path.join(os.tmpdir(), "opsagent-auth-dedup-merge-"));
      const previousAgentDir = process.env.OPSAGENT_AGENT_DIR;
      const previousPiAgentDir = process.env.PI_CODING_AGENT_DIR;
      try {
        const mainDir = path.join(root, "main-agent");
