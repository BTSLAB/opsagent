...
/**
 * Cleanup suspended OpsAgent CLI processes that have accumulated.
...
function buildModelAliasLines(cfg?: OpsAgentConfig) {
...
export async function writeCliImages(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "opsagent-cli-images-"));
  const paths: string[] = [];
