...
beforeAll(async () => {
  ...
  tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "opsagent-embedded-agent-"));
...
const ensureModels = (cfg: OpsAgentConfig) => ensureOpsAgentModelsJson(cfg, agentDir) as unknown;
