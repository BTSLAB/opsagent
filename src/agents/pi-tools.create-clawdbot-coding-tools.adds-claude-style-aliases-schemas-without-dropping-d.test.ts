...
const defaultTools = createOpsAgentCodingTools();

describe("createOpsAgentCodingTools", () => {
  it("keeps read tool image metadata intact", async () => {
    const readTool = defaultTools.find((tool) => tool.name === "read");
    expect(readTool).toBeDefined();

    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "opsagent-read-"));
    try {
  ...
  it("returns text content without image blocks for text files", async () => {
    const tools = createOpsAgentCodingTools();
    ...
    try {
      ...
      const contents = "Hello from opsagent read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    const sandbox = {
      ...
      workspaceDir: path.join(os.tmpdir(), "opsagent-sandbox"),
      agentWorkspaceDir: path.join(os.tmpdir(), "opsagent-workspace"),
    ...
    const tools = createOpsAgentCodingTools({ sandbox });
