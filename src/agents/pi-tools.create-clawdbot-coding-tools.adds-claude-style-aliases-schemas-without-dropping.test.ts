...
describe("createOpsAgentCodingTools", () => {
  ...
  it("keeps raw core tool schemas union-free", () => {
    const tools = createOpsAgentTools();
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createOpsAgentCodingTools({ messageProvider: "discord" });
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "opsagent-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "opsagent-outside.txt");
