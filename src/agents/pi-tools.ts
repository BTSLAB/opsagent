...
function resolveExecConfig(cfg: OpsAgentConfig | undefined) {
...
export function createOpsAgentCodingTools(options?: {
  ...
  const base = (codingTools as unknown as AnyAgentTool[]).flatMap((tool) => {
    if (tool.name === readTool.name) {
      ...
      return [createOpsAgentReadTool(freshReadTool)];
    }
