...
import { createOpsAgentCodingTools } from "./pi-tools.js";
...
describe("workspace path resolution", () => {
  it("reads relative paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("opsagent-ws-", async (workspaceDir) => {
      await withTempDir("opsagent-cwd-", async (otherDir) => {
        ...
        try {
          const tools = createOpsAgentCodingTools({ workspaceDir });
    ...
    await withTempDir("opsagent-ws-", async (workspaceDir) => {
      await withTempDir("opsagent-cwd-", async (otherDir) => {
        ...
        try {
          ...
          expect(updated).toBe("hello opsagent");
        } finally {
          process.chdir(prevCwd);
        }
      });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("opsagent-ws-", async (workspaceDir) => {
      await withTempDir("opsagent-override-", async (overrideDir) => {
        const tools = createOpsAgentCodingTools({ workspaceDir });
        const execTool = tools.find((tool) => tool.name === "exec");
        expect(execTool).toBeDefined();
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("opsagent-sandbox-", async (sandboxDir) => {
      await withTempDir("opsagent-workspace-", async (workspaceDir) => {
        const sandbox = {
          enabled: true,
          ...
          workspaceAccess: "rw",
          containerName: "opsagent-sbx-test",
          containerWorkdir: "/workspace",
          docker: {
            image: "opsagent-sandbox:bookworm-slim",
            containerPrefix: "opsagent-sbx-",
            workdir: "/workspace",
            readOnlyRoot: true,
            tmpfs: [],
            network: "none",
        ...
        const tools = createOpsAgentCodingTools({ workspaceDir, sandbox });
        const readTool = tools.find((tool) => tool.name === "read");
        const writeTool = tools.find((tool) => tool.name === "write");
        const editTool = tools.find((tool) => tool.name === "edit");
