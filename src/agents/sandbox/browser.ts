...
export async function ensureSandboxBrowser(params: {
  ...
  if (!state.exists) {
    ...
    const args = buildSandboxCreateArgs({
      ...
      labels: { "opsagent.sandboxBrowser": "1" },
    });
    ...
    args.push("-e", `OPSAGENT_BROWSER_HEADLESS=${params.cfg.browser.headless ? "1" : "0"}`);
    args.push("-e", `OPSAGENT_BROWSER_ENABLE_NOVNC=${params.cfg.browser.enableNoVnc ? "1" : "0"}`);
    args.push("-e", `OPSAGENT_BROWSER_CDP_PORT=${params.cfg.browser.cdpPort}`);
    args.push("-e", `OPSAGENT_BROWSER_VNC_PORT=${params.cfg.browser.vncPort}`);
    args.push("-e", `OPSAGENT_BROWSER_NOVNC_PORT=${params.cfg.browser.noVncPort}`);
