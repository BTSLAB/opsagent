...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "opsagent.sandbox=1");
  args.push("--label", `opsagent.sessionKey=${params.scopeKey}`);
  args.push("--label", `opsagent.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `opsagent.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  const result = await execDocker(
    ["inspect", "-f", '{{ index .Config.Labels "opsagent.configHash" }}', containerName],
    { allowFailure: true },
  );
  if (result.code !== 0) return null;
...
function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`opsagent sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`opsagent sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("opsagent sandbox recreate --all");
}
