...
type WebSearchConfig = NonNullable<OpsAgentConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: OpsAgentConfig): WebSearchConfig {
...
function missingSearchKeyPayload(provider: (typeof SEARCH_PROVIDERS)[number]) {
  ...
  return {
    ...
    message: `web_search needs a Brave Search API key. Run \`${formatCliCommand("opsagent configure --section web")}\` to store it, or set BRAVE_API_KEY in the Gateway environment.`,
    docs: "https://docs.opsagent.dev/tools/web",
  };
}
...
async function runPerplexitySearch(params: {
  ...
  const res = await fetch(endpoint, {
    ...
    headers: {
      ...
      "HTTP-Referer": "https://opsagent.com",
      "X-Title": "OpsAgent Web Search",
    },
    body: JSON.stringify({
      model: params.model,
      messages: [
