...
import { scheduleGatewaySigusr1Restart, triggerOpsAgentRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting opsagent in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerOpsAgentRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting opsagent via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
