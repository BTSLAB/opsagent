...
beforeAll(async () => {
  testWorkspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "opsagent-commands-"));
...
function buildParams(commandBody: string, cfg: OpsAgentConfig, ctxOverrides?: Partial<MsgContext>) {
  const ctx = {
    Body: commandBody,
