...
type AgentConfig = Partial<NonNullable<NonNullable<OpsAgentConfig["agents"]>["defaults"]>>;
...
const readUsageFromSessionLog = (
  ...
  | undefined => {
  // Transcripts are stored at the session file path (fallback: ~/.opsagent/sessions/<SessionId>.jsonl)
...
export function buildStatusMessage(args: StatusArgs): string {
  ...
  const versionLine = `🦞 OpsAgent ${VERSION}${commit ? ` (${commit})` : ""}`;
...
export function buildHelpMessage(cfg?: OpsAgentConfig): string {
  const options = [
    "/think <level>",
    "/verbose on|full|off",
    "/reasoning on|off",
