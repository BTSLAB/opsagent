...
function getBrowserControlToken(): string | null {
  const env = process.env.OPSAGENT_BROWSER_CONTROL_TOKEN?.trim();
  if (env) return env;
...
function enhanceBrowserFetchError(url: string, err: unknown, timeoutMs: number): Error {
  ...
  const hint = `Start (or restart) the OpsAgent gateway (OpsAgent.app menubar, or \`${formatCliCommand("opsagent gateway")}\`) and try again.`;
