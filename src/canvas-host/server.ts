...
(() => {
  ...
  const hasIOS = () => !!(window.webkit && window.webkit.messageHandlers && window.webkit.messageHandlers.opsagentCanvasA2UIAction);
  const hasAndroid = () => !!(window.opsagentCanvasA2UIAction && typeof window.opsagentCanvasA2UIAction.postMessage === "function");
  const hasHelper = () => typeof window.opsagentSendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
  ...
  window.addEventListener("opsagent:a2ui-action-status", (ev) => {
  ...
  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android OpsAgent node canvas.");
      return;
    }
    const ok = window.opsagentSendUserAction({
      name,
      surfaceId: "main",
      sourceComponentId,
      context: { t: Date.now() },
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.OPSAGENT_SKIP_CANVAS_HOST)) return true;
    ...
    try {
      ...
      if (!filePath) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>OpsAgent Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
