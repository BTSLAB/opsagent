...
function setWhatsAppDmPolicy(cfg: OpsAgentConfig, dmPolicy: DmPolicy): OpsAgentConfig {
  return mergeWhatsAppConfig(cfg, { dmPolicy });
}

function setWhatsAppAllowFrom(cfg: OpsAgentConfig, allowFrom?: string[]): OpsAgentConfig {
  return mergeWhatsAppConfig(cfg, { allowFrom }, { unsetOnUndefined: ["allowFrom"] });
}

function setWhatsAppSelfChatMode(cfg: OpsAgentConfig, selfChatMode: boolean): OpsAgentConfig {
...
async function detectWhatsAppLinked(cfg: OpsAgentConfig, accountId: string): Promise<boolean> {
...
async function promptWhatsAppAllowFrom(
  ...
  if (options?.forceAllowlist) {
    await prompter.note(
      "We need the sender/owner number so OpsAgent can allowlist you.",
      "WhatsApp number",
    );
  ...
  const phoneMode = (await prompter.select({
    ...
    options: [
      ...
      { value: "separate", label: "Separate phone just for OpsAgent" },
    ],
...
export const whatsappOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  configure: async ({
    ...
    if (wantsLink) {
      ...
      await prompter.note(
        `Run \`${formatCliCommand("opsagent channels login")}\` later to link WhatsApp.`,
        "WhatsApp",
      );
    }
