...
describe("browser extension install", () => {
  it("installs into the state dir (never node_modules)", async () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "opsagent-ext-"));
  ...
  it("copies extension path to clipboard", async () => {
    ...
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "opsagent-ext-path-"));
    ...
    try {
      ...
      if (prev === undefined) delete process.env.OPSAGENT_STATE_DIR;
      else process.env.OPSAGENT_STATE_DIR = prev;
    }
  });
});
