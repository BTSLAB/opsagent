...
import { STATE_DIR_OPSAGENT } from "../config/paths.js";
...
function installedExtensionRootDir() {
  return path.join(STATE_DIR_OPSAGENT, "browser", "chrome-extension");
}
...
export async function installChromeExtension(opts?: {
  ...
  if (!hasManifest(src)) {
    throw new Error("Bundled Chrome extension is missing. Reinstall OpsAgent and try again.");
  }

  const stateDir = opts?.stateDir ?? STATE_DIR_OPSAGENT;
      ...
      defaultRuntime.error(
        info(
          [
            ...
            `- Pin “OpsAgent Browser Relay”, then click it on the tab (badge shows ON)`,
            "",
            `${theme.muted("Docs:")} ${formatDocsLink("/tools/chrome-extension", "docs.opsagent.dev/tools/chrome-extension")}`,
          ].join("\n"),
        ),
      );
    });
    ...
    .action(async (_opts, cmd) => {
      ...
      if (!hasManifest(dir)) {
        defaultRuntime.error(
          danger(
            [
              `Chrome extension is not installed. Run: "${formatCliCommand("opsagent browser extension install")}"`,
              `Docs: ${formatDocsLink("/tools/chrome-extension", "docs.opsagent.dev/tools/chrome-extension")}`,
            ].join("\n"),
          ),
        );
        defaultRuntime.exit(1);
