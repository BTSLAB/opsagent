...
describe("gateway SIGTERM", () => {
  ...
  it("exits 0 on SIGTERM", { timeout: 180_000 }, async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "opsagent-gateway-test-"));
    const out: string[] = [];
    const err: string[] = [];
    ...
    const env = {
      ...process.env,
      OPSAGENT_NO_RESPAWN: "1",
      OPSAGENT_STATE_DIR: stateDir,
      OPSAGENT_SKIP_CHANNELS: "1",
      OPSAGENT_SKIP_GMAIL_WATCHER: "1",
      OPSAGENT_SKIP_CRON: "1",
      OPSAGENT_SKIP_BROWSER_CONTROL_SERVER: "1",
      OPSAGENT_SKIP_CANVAS_HOST: "1",
    };
    const bootstrapPath = path.join(stateDir, "opsagent-entry-bootstrap.mjs");
