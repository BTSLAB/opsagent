...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "docs.opsagent.dev/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + OpsAgent hooks")
    ...
    .option("--hook-url <url>", "OpsAgent hook URL")
    .option("--hook-token <token>", "OpsAgent hook token")
