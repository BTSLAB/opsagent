...
export function listAgentEntries(cfg: OpsAgentConfig): AgentEntry[] {
...
function resolveAgentName(cfg: OpsAgentConfig, agentId: string) {
...
function resolveAgentModel(cfg: OpsAgentConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: OpsAgentConfig): AgentSummary[] {
