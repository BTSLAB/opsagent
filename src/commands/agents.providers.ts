...
function resolveDefaultAccountId(cfg: OpsAgentConfig, provider: ChannelId): string {
  const plugin = getChannelPlugin(provider);
  if (!plugin) return DEFAULT_ACCOUNT_ID;
  return resolveChannelDefaultAccountId({ plugin, cfg });
}

function shouldShowProviderEntry(entry: ProviderAccountStatus, cfg: OpsAgentConfig): boolean {
...
export function summarizeBindings(cfg: OpsAgentConfig, bindings: AgentBinding[]): string[] {
