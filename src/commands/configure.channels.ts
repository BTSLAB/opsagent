...
import { CONFIG_PATH_OPSAGENT } from "../config/config.js";
...
export async function removeChannelConfigWizard(
  ...
  while (true) {
    ...
    if (configured.length === 0) {
      note(
        [
          "No channel config found in opsagent.json.",
          `Tip: \`${formatCliCommand("opsagent channels status")}\` shows what is configured and enabled.`,
        ].join("\n"),
        "Remove channel",
      );
      return next;
    ...
    const confirmed = guardCancel(
      await confirm({
        message: `Delete ${label} configuration from ${shortenHomePath(CONFIG_PATH_OPSAGENT)}?`,
        initialValue: false,
      }),
      runtime,
    );
    ...
    next = {
      ...
      channels: Object.keys(nextChannels).length
        ? (nextChannels as OpsAgentConfig["channels"])
        : undefined,
    };
