...
function formatAuthIssueHint(issue: AuthIssue): string | null {
  ...
  if (issue.provider === "openai-codex" && issue.profileId === CODEX_CLI_PROFILE_ID) {
    return `Run \`codex login\` (or \`${formatCliCommand("opsagent configure")}\` → OpenAI Codex OAuth).`;
  }
  return `Re-auth via \`${formatCliCommand("opsagent configure")}\` or \`${formatCliCommand("opsagent onboard")}\`.`;
}
