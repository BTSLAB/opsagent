...
function stripUnknownConfigKeys(config: OpsAgentConfig): {
  config: OpsAgentConfig;
  removed: string[];
} {
  const parsed = OpsAgentSchema.safeParse(config);
  if (parsed.success) {
    return { config, removed: [] };
  }

  const next = structuredClone(config) as OpsAgentConfig;
...
function noteOpencodeProviderOverrides(cfg: OpsAgentConfig) {
...
export async function loadAndMaybeMigrateDoctorConfig(params: {
  ...
  let cfg: OpsAgentConfig = baseCfg;
  let candidate = structuredClone(baseCfg) as OpsAgentConfig;
  ...
  if (snapshot.legacyIssues.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(
        `Run "${formatCliCommand("opsagent doctor --fix")}" to apply legacy migrations.`,
      );
    }
  }
  ...
  if (normalized.changes.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(`Run "${formatCliCommand("opsagent doctor --fix")}" to apply these changes.`);
    }
  }
  ...
  if (unknown.removed.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push('Run "opsagent doctor --fix" to remove these keys.');
    }
  }
  ...
  return { cfg, path: snapshot.path ?? CONFIG_PATH_OPSAGENT, shouldWriteConfig };
}
