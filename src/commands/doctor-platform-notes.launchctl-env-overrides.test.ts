...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "OPSAGENT_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("OPSAGENT_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv OPSAGENT_GATEWAY_TOKEN");
    expect(message).not.toContain("OPSAGENT_GATEWAY_PASSWORD");
  });
