...
import { CONFIG_PATH_OPSAGENT, readConfigFileSnapshot, writeConfigFile } from "../config/config.js";
...
function resolveMode(cfg: OpsAgentConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  intro("OpsAgent doctor");

  const root = await resolveOpsAgentPackageRoot({
  ...
  let cfg: OpsAgentConfig = configResult.cfg;

  const configPath = configResult.path ?? CONFIG_PATH_OPSAGENT;
  if (!cfg.gateway?.mode) {
    const lines = [
      "gateway.mode is unset; gateway start will be blocked.",
      `Fix: run ${formatCliCommand("opsagent configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("opsagent config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("opsagent setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  await noteStateIntegrity(cfg, prompter, configResult.path ?? CONFIG_PATH_OPSAGENT);
  ...
  if (shouldWriteConfig) {
    ...
    const backupPath = `${CONFIG_PATH_OPSAGENT}.bak`;
    if (fs.existsSync(backupPath)) {
      runtime.log(`Backup: ${shortenHomePath(backupPath)}`);
    }
  } else {
    runtime.log(`Run "${formatCliCommand("opsagent doctor --fix")}" to apply changes.`);
  }
