...
beforeEach(() => {
  ...
  resolveOpsAgentPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.OPSAGENT_STATE_DIR;
  originalUpdateInProgress = process.env.OPSAGENT_UPDATE_IN_PROGRESS;
  process.env.OPSAGENT_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "opsagent-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.OPSAGENT_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.OPSAGENT_UPDATE_IN_PROGRESS;
  } else {
    process.env.OPSAGENT_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
...
const resolveOpsAgentPackageRoot = vi.fn().mockResolvedValue(null);
...
vi.mock("../plugins/loader.js", () => ({
  loadOpsAgentPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
vi.mock("../config/config.js", async (importOriginal) => {
  ...
  return {
    ...actual,
    CONFIG_PATH_OPSAGENT: "/tmp/opsagent.json",
    createConfigIO,
...
vi.mock("../infra/opsagent-root.js", () => ({
  resolveOpsAgentPackageRoot,
}));
...
describe("doctor command", () => {
  it("warns when the state directory is missing", async () => {
    ...
    const missingDir = fs.mkdtempSync(path.join(os.tmpdir(), "opsagent-missing-state-"));
    fs.rmSync(missingDir, { recursive: true, force: true });
    process.env.OPSAGENT_STATE_DIR = missingDir;
    note.mockClear();
  ...
  it("skips gateway auth warning when OPSAGENT_GATEWAY_TOKEN is set", async () => {
    readConfigFileSnapshot.mockResolvedValue({
      path: "/tmp/opsagent.json",
      exists: true,
      raw: "{}",
    ...
    process.env.OPSAGENT_GATEWAY_TOKEN = "env-token-1234567890";
    note.mockClear();
