...
import { readConfigFileSnapshot, type OpsAgentConfig } from "../../config/config.js";
...
function applyDefaultModel(cfg: OpsAgentConfig, model: string): OpsAgentConfig {
...
export async function modelsAuthLoginCommand(opts: LoginOptions, runtime: RuntimeEnv) {
  ...
  if (providers.length === 0) {
    throw new Error(
      `No provider plugins found. Install one via \`${formatCliCommand("opsagent plugins install")}\`.`,
    );
  }
