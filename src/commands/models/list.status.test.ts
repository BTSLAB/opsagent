...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveOpsAgentAgentDir: vi.fn().mockReturnValue("/tmp/opsagent-agent"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveOpsAgentAgentDir: mocks.resolveOpsAgentAgentDir,
}));
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(payload.auth.storePath).toBe("/tmp/opsagent-agent/auth-profiles.json");
