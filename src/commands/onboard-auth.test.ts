...
const requireAgentDir = () => {
  const agentDir = process.env.OPSAGENT_AGENT_DIR;
  if (!agentDir) throw new Error("OPSAGENT_AGENT_DIR not set");
  return agentDir;
};
...
describe("writeOAuthCredentials", () => {
  ...
  afterEach(async () => {
    ...
    delete process.env.OPSAGENT_OAUTH_DIR;
  });

  it("writes auth-profiles.json under OPSAGENT_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "opsagent-oauth-"));
    process.env.OPSAGENT_STATE_DIR = tempStateDir;
    process.env.OPSAGENT_AGENT_DIR = path.join(tempStateDir, "agent");
    process.env.PI_CODING_AGENT_DIR = process.env.OPSAGENT_AGENT_DIR;
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to OPSAGENT_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "opsagent-minimax-"));
    process.env.OPSAGENT_STATE_DIR = tempStateDir;
    process.env.OPSAGENT_AGENT_DIR = path.join(tempStateDir, "custom-agent");
    process.env.PI_CODING_AGENT_DIR = process.env.OPSAGENT_AGENT_DIR;
