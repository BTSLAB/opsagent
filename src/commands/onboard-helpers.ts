...
export function summarizeExistingConfig(config: OpsAgentConfig): string {
  const rows: string[] = [];
...
export function formatControlUiSshHint(params: {
  ...
  return [
    ...
    authedUrl,
    "Docs:",
    "https://docs.opsagent.dev/gateway/remote",
    "https://docs.opsagent.dev/web/control-ui",
  ]
    .filter(Boolean)
    .join("\n");
}
...
export async function handleReset(scope: ResetScope, workspaceDir: string, runtime: RuntimeEnv) {
  await moveToTrash(CONFIG_PATH_OPSAGENT, runtime);
