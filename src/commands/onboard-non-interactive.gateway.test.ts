...
describe("onboard (non-interactive): gateway and remote auth", () => {
  const prev = {
    ...
    password: process.env.OPSAGENT_GATEWAY_PASSWORD,
  };
  ...
  const initStateDir = async (prefix: string) => {
    ...
    process.env.OPSAGENT_STATE_DIR = stateDir;
  ...
  beforeAll(async () => {
    ...
    tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "opsagent-onboard-"));
  ...
  afterAll(async () => {
    ...
    process.env.OPSAGENT_GATEWAY_PASSWORD = prev.password;
  });
  ...
  it("auto-enables token auth when binding LAN and persists the token", async () => {
    ...
    process.env.OPSAGENT_STATE_DIR = stateDir;
    process.env.OPSAGENT_CONFIG_PATH = path.join(stateDir, "opsagent.json");
