...
import { type OpsAgentConfig, CONFIG_PATH_OPSAGENT, writeConfigFile } from "../config/config.js";
...
async function readConfigFileRaw(): Promise<{
  exists: boolean;
  parsed: OpsAgentConfig;
}> {
  try {
    const raw = await fs.readFile(CONFIG_PATH_OPSAGENT, "utf-8");
    const parsed = JSON5.parse(raw);
    if (parsed && typeof parsed === "object") {
      return { exists: true, parsed: parsed as OpsAgentConfig };
    }
