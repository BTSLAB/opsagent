...
export async function statusCommand(
  ...
  runtime.log(theme.heading("OpsAgent status"));
  runtime.log("");
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("opsagent security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("opsagent security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.opsagent.dev/faq");
  runtime.log("Troubleshooting: https://docs.opsagent.dev/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("opsagent status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("opsagent logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("opsagent status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("opsagent gateway probe")}`);
  }
}
