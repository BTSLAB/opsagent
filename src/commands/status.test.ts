...
vi.mock("../infra/opsagent-root.js", () => ({
  resolveOpsAgentPackageRoot: vi.fn().mockResolvedValue("/tmp/opsagent"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/opsagent/pnpm-lock.yaml",
      markerPath: "/tmp/opsagent/node_modules/.modules.yaml",
    },
...
vi.mock("../daemon/service.js", () => ({
  resolveGatewayService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/com.opsagent.gateway.plist",
    }),
  }),
}));
vi.mock("../daemon/node-service.js", () => ({
  resolveNodeService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/com.opsagent.node.plist",
    }),
  }),
}));
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(logs.some((l) => l.includes("OpsAgent status"))).toBe(true);
    ...
    expect(logs.some((l) => l.includes("opsagent status --all"))).toBe(true);
  });

  it("shows gateway auth when reachable", async () => {
    const prevToken = process.env.OPSAGENT_GATEWAY_TOKEN;
    process.env.OPSAGENT_GATEWAY_TOKEN = "abcd1234";
    try {
