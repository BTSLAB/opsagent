...
const ETC_OPSAGENT_DIR = path.join(ROOT_DIR, "etc", "opsagent");
const SHARED_DIR = path.join(ROOT_DIR, "shared");

const DEFAULT_BASE_PATH = path.join(CONFIG_DIR, "opsagent.json");

function configPath(...parts: string[]) {
  return path.join(CONFIG_DIR, ...parts);
}

function etcOpsAgentPath(...parts: string[]) {
  return path.join(ETC_OPSAGENT_DIR, ...parts);
}
...
describe("resolveConfigIncludes", () => {
  ...
  it("resolves absolute path $include", () => {
    const absolute = etcOpsAgentPath("agents.json");
  ...
  it("resolves parent directory references", () => {
    ...
    expect(resolve(obj, files, configPath("sub", "opsagent.json"))).toEqual({
      shared: true,
    });
  });
});
