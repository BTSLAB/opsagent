...
import type { OpsAgentConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import { validateConfigObjectWithPlugins } from "./validation.js";
import { compareOpsAgentVersions } from "./version.js";
...
function coerceConfig(value: unknown): OpsAgentConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as OpsAgentConfig;
}
...
function stampConfigVersion(cfg: OpsAgentConfig): OpsAgentConfig {
...
function warnIfConfigFromFuture(cfg: OpsAgentConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) return;
  const cmp = compareOpsAgentVersions(VERSION, touched);
  if (cmp === null) return;
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer OpsAgent (${touched}); current version is ${VERSION}.`,
    );
  }
}

function applyConfigEnv(cfg: OpsAgentConfig, env: NodeJS.ProcessEnv): void {
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): OpsAgentConfig {
    try {
      ...
      if (resolved && typeof resolved === "object" && "env" in resolved) {
        applyConfigEnv(resolved as OpsAgentConfig, deps.env);
      }
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as OpsAgentConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: OpsAgentConfig) {
    clearConfigCache();
...
// module scope. `OPSAGENT_CONFIG_PATH` (and friends) are expected to work even
...
function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.OPSAGENT_CONFIG_CACHE_MS?.trim();
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.OPSAGENT_DISABLE_CONFIG_CACHE?.trim()) return false;
...
export function loadConfig(): OpsAgentConfig {
...
export async function writeConfigFile(cfg: OpsAgentConfig): Promise<void> {
  clearConfigCache();
