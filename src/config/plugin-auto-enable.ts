import type { OpsAgentConfig } from "./config.js";
import {
  getChatChannelMeta,
  listChatChannels,
...
function isTelegramConfigured(cfg: OpsAgentConfig, env: NodeJS.ProcessEnv): boolean {
...
function isDiscordConfigured(cfg: OpsAgentConfig, env: NodeJS.ProcessEnv): boolean {
...
function isSlackConfigured(cfg: OpsAgentConfig, env: NodeJS.ProcessEnv): boolean {
  if (
...
function isSignalConfigured(cfg: OpsAgentConfig): boolean {
...
function isIMessageConfigured(cfg: OpsAgentConfig): boolean {
...
function isWhatsAppConfigured(cfg: OpsAgentConfig): boolean {
...
function isGenericChannelConfigured(cfg: OpsAgentConfig, channelId: string): boolean {
...
function collectModelRefs(cfg: OpsAgentConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: OpsAgentConfig, providerId: string): boolean {
...
function isPluginExplicitlyDisabled(cfg: OpsAgentConfig, pluginId: string): boolean {
  const entry = cfg.plugins?.entries?.[pluginId];
  return entry?.enabled === false;
}

function isPluginDenied(cfg: OpsAgentConfig, pluginId: string): boolean {
...
function enablePluginEntry(cfg: OpsAgentConfig, pluginId: string): OpsAgentConfig {
  const entries = {
    ...cfg.plugins?.entries,
    [pluginId]: {
