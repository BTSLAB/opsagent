...
import { OpsAgentSchema } from "./zod-schema.js";
...
export type ConfigSchema = ReturnType<typeof OpsAgentSchema.toJSONSchema>;
...
const FIELD_HELP: Record<string, string> = {
  "meta.lastTouchedVersion": "Auto-set when OpsAgent writes the config.",
  ...
  "gateway.controlUi.basePath":
    "Optional URL prefix where the Control UI is served (e.g. /opsagent).",
  ...
  "diagnostics.cacheTrace.filePath":
    "JSONL output path for cache trace logs (default: $OPSAGENT_STATE_DIR/logs/cache-trace.jsonl).",
  ...
  "agents.defaults.memorySearch.store.path":
    "SQLite index path (default: ~/.opsagent/memory/{agentId}.sqlite).",
  ...
  "plugins.installs":
    "CLI-managed install metadata (used by `opsagent plugins update` to locate install sources).",
  "plugins.installs.*.source": 'Install source ("npm", "archive", or "path").',
  "plugins.installs.*.spec": "Original npm spec used for install (if source is npm).",
  "plugins.installs.*.sourcePath": "Original archive/path used for install (if any).",
  "plugins.installs.*.installPath":
    "Resolved install directory (usually ~/.opsagent/extensions/<id>).",
...
const FIELD_PLACEHOLDERS: Record<string, string> = {
  ...
  "gateway.controlUi.basePath": "/opsagent",
...
function buildBaseConfigSchema(): ConfigSchemaResponse {
  ...
  const schema = OpsAgentSchema.toJSONSchema({
    target: "draft-07",
    unrepresentable: "any",
  });
  schema.title = "OpsAgentConfig";
