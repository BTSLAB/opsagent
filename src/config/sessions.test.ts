...
describe("sessions", () => {
  ...
  it("derives session transcripts dir from OPSAGENT_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { OPSAGENT_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("includes topic ids in session transcript filenames", () => {
    ...
    process.env.OPSAGENT_STATE_DIR = "/custom/state";
    try {
