...
import type { OpsAgentConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: OpsAgentConfig): ConfigValidationIssue[] {
...
export function validateConfigObject(
  raw: unknown,
): { ok: true; config: OpsAgentConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = OpsAgentSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as OpsAgentConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as OpsAgentConfig);
  ...
  return {
    ...
    config: applyModelDefaults(
      applyAgentDefaults(applySessionDefaults(validated.data as OpsAgentConfig)),
    ),
  };
}
