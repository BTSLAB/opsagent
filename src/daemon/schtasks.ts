...
function resolveTaskName(env: Record<string, string | undefined>): string {
  const override = env.OPSAGENT_WINDOWS_TASK_NAME?.trim();
  if (override) return override;
  return resolveGatewayWindowsTaskName(env.OPSAGENT_PROFILE);
}

export function resolveTaskScriptPath(env: Record<string, string | undefined>): string {
  const override = env.OPSAGENT_TASK_SCRIPT?.trim();
  if (override) return override;
  const scriptName = env.OPSAGENT_TASK_SCRIPT_NAME?.trim() || "gateway.cmd";
...
export async function installScheduledTask({
  ...
  const taskDescription =
    ...
    formatGatewayServiceDescription({
      profile: env.OPSAGENT_PROFILE,
      version: environment?.OPSAGENT_SERVICE_VERSION ?? env.OPSAGENT_SERVICE_VERSION,
    });
