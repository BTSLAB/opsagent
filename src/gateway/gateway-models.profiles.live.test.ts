...
import { resolveOpsAgentAgentDir } from "../agents/agent-paths.js";
...
import { ensureOpsAgentModelsJson } from "../agents/models-config.js";
import { loadConfig } from "../config/config.js";
import type { OpsAgentConfig, ModelProviderConfig } from "../config/types.js";
...
const GATEWAY_LIVE = isTruthyEnvValue(process.env.OPSAGENT_LIVE_GATEWAY);
const ZAI_FALLBACK = isTruthyEnvValue(process.env.OPSAGENT_LIVE_GATEWAY_ZAI_FALLBACK);
const PROVIDERS = parseFilter(process.env.OPSAGENT_LIVE_GATEWAY_PROVIDERS);
...
function sanitizeAuthConfig(params: {
  ...
  let profiles: NonNullable<OpsAgentConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  const previous = {
    ...
    agentDir: process.env.OPSAGENT_AGENT_DIR,
    piAgentDir: process.env.PI_CODING_AGENT_DIR,
    stateDir: process.env.OPSAGENT_STATE_DIR,
  };
  ...
  const hostAgentDir = resolveOpsAgentAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "opsagent-live-state-"));
  ...
  process.env.OPSAGENT_AGENT_DIR = tempAgentDir;
  ...
  const toolProbePath = path.join(workspaceDir, `.opsagent-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolveOpsAgentAgentDir();
  const sanitizedCfg: OpsAgentConfig = {
    ...params.cfg,
  ...
  try {
        ...
        try {
          ...
          const toolProbe = await client.request<AgentFinalPayload>(
            "agent",
            {
              ...
              message:
                "OpsAgent live tool probe (local, safe): " +
    ...
    process.env.OPSAGENT_CONFIG_PATH = previous.configPath;
    process.env.OPSAGENT_GATEWAY_TOKEN = previous.token;
    process.env.OPSAGENT_SKIP_CHANNELS = previous.skipChannels;
    process.env.OPSAGENT_SKIP_GMAIL_WATCHER = previous.skipGmail;
    process.env.OPSAGENT_SKIP_CRON = previous.skipCron;
    process.env.OPSAGENT_SKIP_CANVAS_HOST = previous.skipCanvas;
    process.env.OPSAGENT_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.OPSAGENT_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      await ensureOpsAgentModelsJson(cfg);
      ...
      const rawModels = process.env.OPSAGENT_LIVE_GATEWAY_MODELS?.trim();
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.opsagent-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
