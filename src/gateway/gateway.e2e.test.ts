...
describe("gateway e2e", () => {
  it(
    ...
    async () => {
      const prev = {
        ...
        skipBrowser: process.env.OPSAGENT_SKIP_BROWSER_CONTROL_SERVER,
      };

      const { baseUrl: openaiBaseUrl, restore } = installOpenAiResponsesMock();

      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "opsagent-gw-mock-home-"));
      ...
      const toolProbePath = path.join(workspaceDir, `.opsagent-tool-probe.${nonceA}.txt`);
      await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

      const configDir = path.join(tempHome, ".opsagent");
      await fs.mkdir(configDir, { recursive: true });
      const configPath = path.join(configDir, "opsagent.json");
      ...
      process.env.OPSAGENT_CONFIG_PATH = configPath;
      ...
      try {
        ...
        process.env.OPSAGENT_CONFIG_PATH = prev.configPath;
        process.env.OPSAGENT_GATEWAY_TOKEN = prev.token;
        process.env.OPSAGENT_SKIP_CHANNELS = prev.skipChannels;
        process.env.OPSAGENT_SKIP_GMAIL_WATCHER = prev.skipGmail;
        process.env.OPSAGENT_SKIP_CRON = prev.skipCron;
        process.env.OPSAGENT_SKIP_CANVAS_HOST = prev.skipCanvas;
        process.env.OPSAGENT_SKIP_BROWSER_CONTROL_SERVER = prev.skipBrowser;
      }
    },
  );
  ...
  it("runs wizard over ws and writes auth token config", { timeout: 90_000 }, async () => {
    ...
    const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "opsagent-wizard-home-"));
    process.env.HOME = tempHome;
    ...
    try {
      ...
      await fs.rm(tempHome, { recursive: true, force: true });
      process.env.HOME = prev.home;
      process.env.OPSAGENT_STATE_DIR = prev.stateDir;
      process.env.OPSAGENT_CONFIG_PATH = prev.configPath;
      process.env.OPSAGENT_GATEWAY_TOKEN = prev.token;
      process.env.OPSAGENT_SKIP_CHANNELS = prev.skipChannels;
      process.env.OPSAGENT_SKIP_GMAIL_WATCHER = prev.skipGmail;
