...
export function resolveHooksConfig(cfg: OpsAgentConfig): HooksConfigResolved | null {
...
export function extractHookToken(req: IncomingMessage, url: URL): HookTokenResult {
  ...
  const headerToken =
    typeof req.headers["x-opsagent-token"] === "string"
      ? req.headers["x-opsagent-token"].trim()
      : "";
