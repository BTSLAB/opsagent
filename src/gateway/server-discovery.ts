...
export function formatBonjourInstanceName(displayName: string) {
  ...
  if (!trimmed) return "OpsAgent";
  if (/opsagent/i.test(trimmed)) return trimmed;
  return `${trimmed} (OpsAgent)`;
}

export function resolveBonjourCliPath(opts: ResolveBonjourCliPathOptions = {}): string | undefined {
  const env = opts.env ?? process.env;
  const envPath = env.OPSAGENT_CLI_PATH?.trim();
  ...
  const siblingCli = path.join(execDir, "opsagent");
  ...
  const binCli = path.join(cwd, "bin", "opsagent.js");
...
export async function resolveTailnetDnsHint(opts?: {
  ...
  const env = opts?.env ?? process.env;
  const envRaw = env.OPSAGENT_TAILNET_DNS?.trim();
