...
describe("gateway server auth/connect", () => {
  describe("default auth (token)", () => {
    ...
    test("closes silent handshakes after timeout", { timeout: 60_000 }, async () => {
      vi.useRealTimers();
      const prevHandshakeTimeout = process.env.OPSAGENT_TEST_HANDSHAKE_TIMEOUT_MS;
      process.env.OPSAGENT_TEST_HANDSHAKE_TIMEOUT_MS = "50";
      try {
        ...
        if (prevHandshakeTimeout === undefined) {
          delete process.env.OPSAGENT_TEST_HANDSHAKE_TIMEOUT_MS;
        } else {
          process.env.OPSAGENT_TEST_HANDSHAKE_TIMEOUT_MS = prevHandshakeTimeout;
        }
      }
    });

    test("connect (req) handshake returns hello-ok payload", async () => {
      const { CONFIG_PATH_OPSAGENT, STATE_DIR_OPSAGENT } = await import("../config/config.js");
      const ws = await openWs(port);
      ...
      expect(payload?.snapshot?.configPath).toBe(CONFIG_PATH_OPSAGENT);
      expect(payload?.snapshot?.stateDir).toBe(STATE_DIR_OPSAGENT);
  ...
  describe("token auth", () => {
    ...
    beforeAll(async () => {
      prevToken = process.env.OPSAGENT_GATEWAY_TOKEN;
  ...
  test("requires pairing for scope upgrades", async () => {
    ...
    const { server, ws, port, prevToken } = await startServerWithClient("secret");
    const identityDir = await mkdtemp(join(tmpdir(), "opsagent-device-scope-"));
