...
This directory contains hooks that ship with OpsAgent. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
```bash
opsagent hooks enable session-memory
```
...
**Output**: `~/.opsagent/logs/commands.log`

**Enable**:

```bash
opsagent hooks enable command-logger
```
...
**Docs**: https://docs.opsagent.dev/hooks/soul-evil

**Enable**:

```bash
opsagent hooks enable soul-evil
```
...
homepage: https://docs.opsagent.dev/hooks#my-hook
metadata:
  { "opsagent": { "emoji": "🔗", "events": ["command:new"], "requires": { "bins": ["node"] } } }
---
# Hook Title
...
- **Managed hooks**: `~/.opsagent/hooks/` (shared across workspaces)
...
```bash
opsagent hooks info session-memory
```

Check hook status:

```bash
opsagent hooks check
```
...
```bash
opsagent hooks enable session-memory
opsagent hooks disable command-logger
```

## Configuration

Hooks can be configured in `~/.opsagent/opsagent.json`:
...
2. Restart gateway: `pkill -9 -f 'opsagent.*gateway' && pnpm opsagent gateway`
3. Enable the hook: `opsagent hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.opsagent.dev/hooks
