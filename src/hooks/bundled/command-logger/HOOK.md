...
homepage: https://docs.opsagent.dev/hooks#command-logger
metadata:
  {
    "opsagent":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.opsagent/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.opsagent/logs/commands.log ~/.opsagent/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/opsagent`:
   ```
   /home/username/.opsagent/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
...
```bash
tail -n 20 ~/.opsagent/logs/commands.log
```

Pretty-print with jq:

```bash
cat ~/.opsagent/logs/commands.log | jq .
```

Filter by action:

```bash
grep '"action":"new"' ~/.opsagent/logs/commands.log | jq .
```
