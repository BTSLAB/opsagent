...
`;
    ...
    expect(parsed.opsagent.emoji).toBe("💾");
    expect(parsed.opsagent.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata as string);
    expect(parsed.opsagent.emoji).toBe("📝");
    expect(parsed.opsagent.events).toEqual(["command"]);
    expect(parsed.opsagent.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.opsagent.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"opsagent": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"opsagent": {"events": ["test"]}}');
  });
...
describe("resolveOpsAgentMetadata", () => {
  it("extracts opsagent metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveOpsAgentMetadata(frontmatter);
  ...
  it("returns undefined when opsagent key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        opsagent: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with OpsAgent" },
            { id: "npm", kind: "npm", package: "@opsagent/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@opsagent/hook");
  });
...
homepage: https://docs.opsagent.dev/hooks#session-memory
metadata:
  {
    "opsagent":
      {
        ...
        "events": ["command:new"],
        "requires": { "config": ["workspace.dir"] },
        "install": [{ "id": "bundled", "kind": "bundled", "label": "Bundled with OpsAgent" }],
      },
  }
---
...
`;
    ...
    const opsagent = resolveOpsAgentMetadata(frontmatter);
    expect(opsagent).toBeDefined();
    expect(opsagent?.emoji).toBe("💾");
    expect(opsagent?.events).toEqual(["command:new"]);
    expect(opsagent?.requires?.config).toEqual(["workspace.dir"]);
    expect(opsagent?.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const opsagent = resolveOpsAgentMetadata(frontmatter);
    expect(opsagent?.emoji).toBe("disk");
    expect(opsagent?.events).toEqual(["command:new"]);
  });
});
