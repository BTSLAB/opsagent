...
export async function runGmailSetup(opts: GmailSetupOptions) {
  ...
  if (!configSnapshot.valid) {
    throw new Error(`Config invalid: ${CONFIG_PATH_OPSAGENT}`);
  }
  ...
  const nextConfig: OpsAgentConfig = {
    ...baseConfig,
    hooks: {
      ...baseConfig.hooks,
      enabled: true,
  ...
  defaultRuntime.log(`- config: ${displayPath(CONFIG_PATH_OPSAGENT)}`);
  defaultRuntime.log(`Next: ${formatCliCommand("opsagent webhooks gmail run")}`);
}
