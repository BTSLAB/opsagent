...
function resolveHookKey(entry: HookEntry): string {
  return entry.opsagent?.hookKey ?? entry.hook.name;
}

function normalizeInstallOptions(entry: HookEntry): HookInstallOption[] {
  const install = entry.opsagent?.install ?? [];
...
function buildHookStatus(
  ...
  const managedByPlugin = entry.hook.source === "opsagent-plugin";
  const disabled = managedByPlugin ? false : hookConfig?.enabled === false;
  const always = entry.opsagent?.always === true;
  const emoji = entry.opsagent?.emoji ?? entry.frontmatter.emoji;
  const homepageRaw =
    entry.opsagent?.homepage ??
    entry.frontmatter.homepage ??
    entry.frontmatter.website ??
    entry.frontmatter.url;
  const homepage = homepageRaw?.trim() ? homepageRaw.trim() : undefined;
  const events = entry.opsagent?.events ?? [];
  ...
  const requiredOs = entry.opsagent?.os ?? [];
