...
async function ensureOpsAgentHooks(manifest: HookPackageManifest) {
  const hooks = manifest.opsagent?.hooks;
  if (!Array.isArray(hooks)) {
    throw new Error("package.json missing opsagent.hooks");
  }
  const list = hooks.map((e) => (typeof e === "string" ? e.trim() : "")).filter(Boolean);
  if (list.length === 0) {
    throw new Error("package.json opsagent.hooks is empty");
  }
  return list;
}
...
async function installHookPackageFromDir(params: {
  ...
  try {
    hookEntries = await ensureOpsAgentHooks(manifest);
  } catch (err) {
...
export async function installHooksFromArchive(params: {
  ...
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "opsagent-hook-"));
...
export async function installHooksFromNpmSpec(params: {
  ...
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "opsagent-hook-pack-"));
