...
import type { OpsAgentPluginApi } from "../plugins/types.js";
...
function resolveHookDir(api: OpsAgentPluginApi, dir: string): string {
  if (path.isAbsolute(dir)) return dir;
  return path.resolve(path.dirname(api.source), dir);
}

function normalizePluginHookEntry(api: OpsAgentPluginApi, entry: HookEntry): HookEntry {
  return {
    ...entry,
    hook: {
      ...entry.hook,
    ...
    opsagent: {
      ...entry.opsagent,
      hookKey: entry.opsagent?.hookKey ?? `${api.id}:${entry.hook.name}`,
      events: entry.opsagent?.events ?? [],
    },
  };
}
...
async function loadHookHandler(
  entry: HookEntry,
  api: OpsAgentPluginApi,
): Promise<InternalHookHandler | null> {
  try {
    const url = pathToFileURL(entry.hook.handlerPath).href;
    const cacheBustedUrl = `${url}?t=${Date.now()}`;
    const mod = (await import(cacheBustedUrl)) as Record<string, unknown>;
    const exportName = entry.opsagent?.export ?? "default";
...
export async function registerPluginHooksFromDir(
  ...
  for (const entry of hooks) {
    ...
    const events = normalizedEntry.opsagent?.events ?? [];
    if (events.length === 0) {
