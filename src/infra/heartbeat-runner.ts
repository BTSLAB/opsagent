...
function resolveActiveHoursTimezone(cfg: OpsAgentConfig, raw?: string): string {
...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: OpsAgentConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: OpsAgentConfig) {
  const list = cfg.agents?.list ?? [];
  return list.some((entry) => Boolean(entry?.heartbeat));
}

export function isHeartbeatEnabledForAgent(cfg: OpsAgentConfig, agentId?: string): boolean {
...
function resolveHeartbeatAgents(cfg: OpsAgentConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: OpsAgentConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: OpsAgentConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: OpsAgentConfig) => {
    if (state.stopped) return;
    const now = Date.now();
