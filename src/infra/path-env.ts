...
function candidateBinDirs(opts: EnsureOpsAgentPathOpts): string[] {
  ...
  // Bundled macOS app: `opsagent` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingOpsAgent = path.join(execDir, "opsagent");
    if (isExecutable(siblingOpsAgent)) candidates.push(execDir);
  } catch {
    // ignore
  }

  // Project-local installs (best effort): if a `node_modules/.bin/opsagent` exists near cwd,
  // include it. This helps when running under launchd or other minimal PATH environments.
  const localBinDir = path.join(cwd, "node_modules", ".bin");
  if (isExecutable(path.join(localBinDir, "opsagent"))) candidates.push(localBinDir);
...
/**
 * Best-effort PATH bootstrap so skills that require the `opsagent` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureOpsAgentCliOnPath(opts: EnsureOpsAgentPathOpts = {}) {
  if (isTruthyEnvValue(process.env.OPSAGENT_PATH_BOOTSTRAPPED)) return;
  process.env.OPSAGENT_PATH_BOOTSTRAPPED = "1";
