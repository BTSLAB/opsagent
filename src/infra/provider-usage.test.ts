...
describe("provider usage loading", () => {
  ...
  it("discovers Claude usage from token auth profiles", async () => {
    await withTempHome(
      ...
      {
        env: {
          OPSAGENT_STATE_DIR: (home) => path.join(home, ".opsagent"),
        },
        prefix: "opsagent-provider-usage-",
      },
    );
  });
  ...
  it("prefers claude-cli token for Anthropic usage snapshots", async () => {
    await withTempHome(
      async () => {
        const stateDir = process.env.OPSAGENT_STATE_DIR;
        if (!stateDir) throw new Error("Missing OPSAGENT_STATE_DIR");
