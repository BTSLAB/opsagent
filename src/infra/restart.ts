...
export function triggerOpsAgentRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.OPSAGENT_SYSTEMD_UNIT,
        process.env.OPSAGENT_PROFILE,
      );
  ...
  const label =
    process.env.OPSAGENT_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.OPSAGENT_PROFILE);
