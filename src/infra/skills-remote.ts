...
function listWorkspaceDirs(cfg: OpsAgentConfig): string[] {
...
function collectRequiredBins(entries: SkillEntry[], targetPlatform: string): string[] {
  ...
  for (const entry of entries) {
    const os = entry.opsagent?.os ?? [];
    if (os.length > 0 && !os.includes(targetPlatform)) continue;
    const required = entry.opsagent?.requires?.bins ?? [];
    const anyBins = entry.opsagent?.requires?.anyBins ?? [];
...
export async function refreshRemoteBinsForConnectedNodes(cfg: OpsAgentConfig) {
  if (!remoteRegistry) return;
