...
export const WIDE_AREA_DISCOVERY_DOMAIN = "opsagent.internal.";
export const WIDE_AREA_ZONE_FILENAME = "opsagent.internal.db";
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*opsagent-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "opsagent";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "opsagent");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "opsagent-gw");
  ...
  records.push(`_opsagent-gw._tcp IN PTR ${instanceLabel}._opsagent-gw._tcp`);
  records.push(`${instanceLabel}._opsagent-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._opsagent-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; opsagent-content-hash: ${contentHash}\n${contentBody}`;
}
