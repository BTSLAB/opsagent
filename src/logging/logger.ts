...
export const DEFAULT_LOG_DIR = "/tmp/opsagent";
export const DEFAULT_LOG_FILE = path.join(DEFAULT_LOG_DIR, "opsagent.log"); // legacy single-file path

const LOG_PREFIX = "opsagent";
...
function resolveSettings(): ResolvedSettings {
  let cfg: OpsAgentConfig["logging"] | undefined =
    (loggingState.overrideSettings as LoggerSettings | null) ?? readLoggingConfig();
  if (!cfg) {
    try {
      const loaded = requireConfig("../config/config.js") as {
        loadConfig?: () => OpsAgentConfig;
      };
