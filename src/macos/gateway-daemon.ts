...
declare const __OPSAGENT_VERSION__: string;

const BUNDLED_VERSION =
  (typeof __OPSAGENT_VERSION__ === "string" && __OPSAGENT_VERSION__) ||
  process.env.OPSAGENT_BUNDLED_VERSION ||
  "0.0.0";
...
async function main() {
  if (hasFlag(args, "--version") || hasFlag(args, "-v")) {
    // Match `opsagent --version` behavior for Swift env/version checks.
  ...
  const portRaw =
    argValue(args, "--port") ??
    process.env.OPSAGENT_GATEWAY_PORT ??
  ...
  const bindRaw =
    argValue(args, "--bind") ??
    process.env.OPSAGENT_GATEWAY_BIND ??
    cfg.gateway?.bind ??
    "loopback";
  const bind =
    bindRaw === "loopback" ||
...
void main().catch((err) => {
  console.error(
    "[opsagent] Gateway daemon failed:",
