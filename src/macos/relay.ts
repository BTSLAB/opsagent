...
declare const __OPSAGENT_VERSION__: string | undefined;
...
async function main() {
  ...
  const { ensureOpsAgentCliOnPath } = await import("../infra/path-env.js");
  ...
  process.on("uncaughtException", (error) => {
    console.error("[opsagent] Uncaught exception:", formatUncaughtError(error));
    process.exit(1);
  });
