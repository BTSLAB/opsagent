...
describe("media store", () => {
  ...
  const snapshotEnv = () => {
    for (const key of ["HOME", "USERPROFILE", "HOMEDRIVE", "HOMEPATH", "OPSAGENT_STATE_DIR"]) {
  ...
  beforeAll(async () => {
    snapshotEnv();
    home = await fs.mkdtemp(path.join(os.tmpdir(), "opsagent-test-home-"));
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.OPSAGENT_STATE_DIR = path.join(home, ".opsagent");
    ...
    await fs.mkdir(path.join(home, ".opsagent"), { recursive: true });
  ...
  it("creates and returns media directory", async () => {
    await withTempStore(async (store, home) => {
      ...
      expect(path.normalize(dir)).toContain(`${path.sep}.opsagent${path.sep}media`);
