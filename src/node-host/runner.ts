...
import { ensureOpsAgentCliOnPath } from "../infra/path-env.js";
...
const execHostEnforced = process.env.OPSAGENT_NODE_EXEC_HOST?.trim().toLowerCase() === "app";
const execHostFallbackAllowed =
  process.env.OPSAGENT_NODE_EXEC_FALLBACK?.trim().toLowerCase() !== "0";
...
function ensureNodePathEnv(): string {
  ensureOpsAgentCliOnPath({ pathEnv: process.env.PATH ?? "" });
...
async function handleInvoke(
  ...
  if (command === "browser.proxy") {
    try {
      ...
      const token =
        process.env.OPSAGENT_BROWSER_CONTROL_TOKEN?.trim() || resolved.controlToken?.trim();
      if (token) {
