...
import type { OpsAgentPackageManifest, PackageManifest } from "./manifest.js";
...
function resolvePackageExtensions(manifest: PackageManifest): string[] {
  const raw = manifest.opsagent?.extensions;
...
function deriveIdHint(params: {
  ...
  // package is scoped (example: @opsagent/voice-call -> voice-call).
...
function addCandidate(params: {
  ...
  params.candidates.push({
    ...
    packageOpsAgent: manifest?.opsagent,
  });
}
...
export function discoverOpsAgentPlugins(params: {
  workspaceDir?: string;
  extraPaths?: string[];
  ...
  if (workspaceDir) {
    ...
    const workspaceExt = path.join(workspaceRoot, ".opsagent", "extensions");
    discoverInDirectory({
      dir: workspaceExt,
      origin: "workspace",
