...
import { loadOpsAgentPlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const tempDirs: string[] = [];
const prevBundledDir = process.env.OPSAGENT_BUNDLED_PLUGINS_DIR;
const EMPTY_PLUGIN_SCHEMA = { type: "object", additionalProperties: false, properties: {} };

function makeTempDir() {
  const dir = path.join(os.tmpdir(), `opsagent-plugin-${randomUUID()}`);
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "opsagent.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
afterEach(() => {
  ...
  if (prevBundledDir === undefined) {
    ...
    process.env.OPSAGENT_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});

describe("loadOpsAgentPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    process.env.OPSAGENT_BUNDLED_PLUGINS_DIR = bundledDir;

    const registry = loadOpsAgentPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
    ...
    const enabledRegistry = loadOpsAgentPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
  ...
  it("preserves package.json metadata for bundled memory plugins", () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "@opsagent/memory-core",
        version: "1.2.3",
        description: "Memory plugin package",
        opsagent: { extensions: ["./index.ts"] },
      }),
      "utf-8",
    );
    writePlugin({
