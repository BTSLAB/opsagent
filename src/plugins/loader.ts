...
import { discoverOpsAgentPlugins } from "./discovery.js";
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: OpsAgentPluginDefinition;
  register?: OpsAgentPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as OpsAgentPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as OpsAgentPluginDefinition;
...
export function loadOpsAgentPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverOpsAgentPlugins({
    workspaceDir: options.workspaceDir,
  ...
  const jiti = createJiti(import.meta.url, {
    ...
    ...(pluginSdkAlias ? { alias: { "opsagent/plugin-sdk": pluginSdkAlias } } : {}),
  });
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: OpsAgentPluginModule | null = null;
    try {
      mod = jiti(candidate.source) as OpsAgentPluginModule;
    } catch (err) {
