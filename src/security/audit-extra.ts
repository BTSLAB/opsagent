...
import type { OpsAgentConfig, ConfigFileSnapshot } from "../config/config.js";
...
function summarizeGroupPolicy(cfg: OpsAgentConfig): {
  open: number;
  allowlist: number;
  other: number;
} {
...
export function collectAttackSurfaceSummaryFindings(cfg: OpsAgentConfig): SecurityAuditFinding[] {
...
export function collectSyncedFolderFindings(params: {
  ...
  if (isProbablySyncedPath(params.stateDir) || isProbablySyncedPath(params.configPath)) {
    findings.push({
      ...
      remediation: `Keep OPSAGENT_STATE_DIR on a local-only volume and re-run "${formatCliCommand("opsagent security audit --fix")}".`,
    });
  }
  return findings;
}
...
export function collectSecretsInConfigFindings(cfg: OpsAgentConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const password =
  ...
  if (password && !looksLikeEnvRef(password)) {
    findings.push({
      ...
      remediation:
        "Prefer OPSAGENT_GATEWAY_PASSWORD (env) and remove gateway.auth.password from disk.",
    });
  }
  ...
  if (browserToken && !looksLikeEnvRef(browserToken)) {
    findings.push({
      ...
      remediation:
        "Prefer OPSAGENT_BROWSER_CONTROL_TOKEN (env) and remove browser.controlToken from disk.",
    });
  }
...
export function collectHooksHardeningFindings(cfg: OpsAgentConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  ...
  const browserToken =
    typeof cfg.browser?.controlToken === "string" && cfg.browser.controlToken.trim()
      ...
      : process.env.OPSAGENT_BROWSER_CONTROL_TOKEN?.trim() || null;
...
function collectModels(cfg: OpsAgentConfig): ModelRef[] {
  const out: ModelRef[] = [];
...
export function collectModelHygieneFindings(cfg: OpsAgentConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
function hasWebSearchKey(cfg: OpsAgentConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebSearchEnabled(cfg: OpsAgentConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebFetchEnabled(cfg: OpsAgentConfig): boolean {
  const enabled = cfg.tools?.web?.fetch?.enabled;
  if (enabled === false) return false;
  return true;
}

function isBrowserEnabled(cfg: OpsAgentConfig): boolean {
  try {
...
function listGroupPolicyOpen(cfg: OpsAgentConfig): string[] {
  const out: string[] = [];
  const channels = cfg.channels as Record<string, unknown> | undefined;
...
export function collectExposureMatrixFindings(cfg: OpsAgentConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
