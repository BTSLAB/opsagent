...
describe("security audit", () => {
  ...
  it("flags remote browser control without token as critical", async () => {
    const prev = process.env.OPSAGENT_BROWSER_CONTROL_TOKEN;
    delete process.env.OPSAGENT_BROWSER_CONTROL_TOKEN;
    try {
      const cfg: OpsAgentConfig = {
        browser: {
      ...
      if (prev === undefined) delete process.env.OPSAGENT_BROWSER_CONTROL_TOKEN;
      else process.env.OPSAGENT_BROWSER_CONTROL_TOKEN = prev;
    }
  });
  ...
  it("warns when multiple DM senders share the main session", async () => {
    const cfg: OpsAgentConfig = { session: { dmScope: "main" } };
  ...
  it("flags Discord native commands without a guild user allowlist", async () => {
    const prevStateDir = process.env.OPSAGENT_STATE_DIR;
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "opsagent-security-audit-discord-"));
    process.env.OPSAGENT_STATE_DIR = tmp;
    ...
    try {
      ...
      if (prevStateDir == null) delete process.env.OPSAGENT_STATE_DIR;
      else process.env.OPSAGENT_STATE_DIR = prevStateDir;
    }
  });
  ...
  it("does not flag Discord slash commands when dm.allowFrom includes a Discord snowflake id", async () => {
    ...
    const tmp = await fs.mkdtemp(
      path.join(os.tmpdir(), "opsagent-security-audit-discord-allowfrom-snowflake-"),
    );
    process.env.OPSAGENT_STATE_DIR = tmp;
    await fs.mkdir(path.join(tmp, "credentials"), { recursive: true, mode: 0o700 });
    try {
  ...
  it("flags Discord slash commands when access-group enforcement is disabled and no users allowlist exists", async () => {
    const prevStateDir = process.env.OPSAGENT_STATE_DIR;
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "opsagent-security-audit-discord-open-"));
    process.env.OPSAGENT_STATE_DIR = tmp;
    await fs.mkdir(path.join(tmp, "credentials"), { recursive: true, mode: 0o700 });
    try {
      const cfg: OpsAgentConfig = {
  ...
  it("flags Slack slash commands without a channel users allowlist", async () => {
    const prevStateDir = process.env.OPSAGENT_STATE_DIR;
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "opsagent-security-audit-slack-"));
    process.env.OPSAGENT_STATE_DIR = tmp;
    await fs.mkdir(path.join(tmp, "credentials"), { recursive: true, mode: 0o700 });
    try {
      const cfg: OpsAgentConfig = {
  ...
  it("flags Slack slash commands when access-group enforcement is disabled", async () => {
    const prevStateDir = process.env.OPSAGENT_STATE_DIR;
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "opsagent-security-audit-slack-open-"));
    process.env.OPSAGENT_STATE_DIR = tmp;
    await fs.mkdir(path.join(tmp, "credentials"), { recursive: true, mode: 0o700 });
    try {
      const cfg: OpsAgentConfig = {
  ...
  it("flags Telegram group commands without a sender allowlist", async () => {
    const prevStateDir = process.env.OPSAGENT_STATE_DIR;
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "opsagent-security-audit-telegram-"));
    process.env.OPSAGENT_STATE_DIR = tmp;
    await fs.mkdir(path.join(tmp, "credentials"), { recursive: true, mode: 0o700 });
    try {
      const cfg: OpsAgentConfig = {
  ...
  it("adds a warning when deep probe fails", async () => {
    const cfg: OpsAgentConfig = { gateway: { mode: "local" } };
  ...
  it("warns when hooks token reuses the gateway env token", async () => {
    const prevToken = process.env.OPSAGENT_GATEWAY_TOKEN;
    process.env.OPSAGENT_GATEWAY_TOKEN = "shared-gateway-token-1234567890";
    const cfg: OpsAgentConfig = {
    ...
    try {
      ...
      if (prevToken === undefined) delete process.env.OPSAGENT_GATEWAY_TOKEN;
      else process.env.OPSAGENT_GATEWAY_TOKEN = prevToken;
    }
  });
  ...
  it("warns when state/config look like a synced folder", async () => {
    ...
    const res = await runSecurityAudit({
      ...
      includeFilesystem: false,
      includeChannelSecurity: false,
      stateDir: "/Users/test/Dropbox/.opsagent",
      configPath: "/Users/test/Dropbox/.opsagent/opsagent.json",
    });
  ...
  it("flags group/world-readable config include files", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "opsagent-security-audit-"));
    ...
    const configPath = path.join(stateDir, "opsagent.json");
    await fs.writeFile(configPath, `{ "$include": "./extra.json5" }\n`, "utf-8");
    await fs.chmod(configPath, 0o600);

    const cfg: OpsAgentConfig = { logging: { redactSensitive: "off" } };
    const res = await runSecurityAudit({
      config: cfg,
  ...
  it("flags extensions without plugins.allow", async () => {
    ...
    try {
      ...
      const res = await runSecurityAudit({
        ...
        includeChannelSecurity: false,
        stateDir,
        configPath: path.join(stateDir, "opsagent.json"),
      });
  ...
  describe("maybeProbeGateway auth selection", () => {
    const originalEnvToken = process.env.OPSAGENT_GATEWAY_TOKEN;
    const originalEnvPassword = process.env.OPSAGENT_GATEWAY_PASSWORD;

    beforeEach(() => {
      delete process.env.OPSAGENT_GATEWAY_TOKEN;
      delete process.env.OPSAGENT_GATEWAY_PASSWORD;
    });
    ...
    afterEach(() => {
      if (originalEnvToken == null) {
        ...
        process.env.OPSAGENT_GATEWAY_TOKEN = originalEnvToken;
      }
      if (originalEnvPassword == null) {
        delete process.env.OPSAGENT_GATEWAY_PASSWORD;
      } else {
        process.env.OPSAGENT_GATEWAY_PASSWORD = originalEnvPassword;
      }
    });
    ...
    it("prefers env token over local config token", async () => {
      process.env.OPSAGENT_GATEWAY_TOKEN = "env-token";
      let capturedAuth: { token?: string; password?: string } | undefined;
      const cfg: OpsAgentConfig = {
        gateway: {
          mode: "local",
    ...
    it("prefers env password over remote password", async () => {
      process.env.OPSAGENT_GATEWAY_PASSWORD = "env-pass";
      let capturedAuth: { token?: string; password?: string } | undefined;
      const cfg: OpsAgentConfig = {
        gateway: {
          mode: "remote",
