  ...
  if (stateDirStat.ok) {
    ...
    if (isWorldWritable(bits)) {
      findings.push({
        ...
        detail: `${params.stateDir} mode=${formatOctal(bits)}; other users can write into your OpsAgent state.`,
      ...
      findings.push({
        ...
        detail: `${params.stateDir} mode=${formatOctal(bits)}; group users can write into your OpsAgent state.`,
        remediation: `chmod 700 ${params.stateDir}`,
      });
...
function collectGatewayConfigFindings(cfg: OpsAgentConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
function collectBrowserControlFindings(cfg: OpsAgentConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  ...
  try {
    ...
    findings.push({
      ...
      detail: String(err),
      remediation: `Fix browser.controlUrl/browser.cdpUrl in ${resolveConfigPath()} and re-run "${formatCliCommand("opsagent security audit --deep")}".`,
    });
    return findings;
  }
  ...
  const envToken = process.env.OPSAGENT_BROWSER_CONTROL_TOKEN?.trim();
  ...
  if (!isLoopback) {
    if (!controlToken) {
      findings.push({
        ...
        detail: `browser.controlUrl is non-loopback (${resolved.controlUrl}) but no browser.controlToken (or OPSAGENT_BROWSER_CONTROL_TOKEN) is configured.`,
        remediation:
          "Set browser.controlToken (or export OPSAGENT_BROWSER_CONTROL_TOKEN) and prefer serving over Tailscale Serve or HTTPS reverse proxy.",
      });
    }
...
function collectLoggingFindings(cfg: OpsAgentConfig): SecurityAuditFinding[] {
...
function collectElevatedFindings(cfg: OpsAgentConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
    ...
    const token =
      mode === "remote"
        ? typeof remote?.token === "string" && remote.token.trim()
          ? remote.token.trim()
          : undefined
        : process.env.OPSAGENT_GATEWAY_TOKEN?.trim() ||
          (typeof authToken === "string" && authToken.trim() ? authToken.trim() : undefined);
    const password =
      process.env.OPSAGENT_GATEWAY_PASSWORD?.trim() ||
      (mode === "remote"
...
export async function runSecurityAudit(opts: SecurityAuditOptions): Promise<SecurityAuditReport> {
  ...
  if (deep?.gateway?.attempted && deep.gateway.ok === false) {
    findings.push({
      ...
      remediation: `Run "${formatCliCommand("opsagent status --all")}" to debug connectivity/auth, then re-run "${formatCliCommand("opsagent security audit --deep")}".`,
    });
  }
