...
export function listSignalAccountIds(cfg: OpsAgentConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultSignalAccountId(cfg: OpsAgentConfig): string {
...
function mergeSignalAccountConfig(cfg: OpsAgentConfig, accountId: string): SignalAccountConfig {
...
export function listEnabledSignalAccounts(cfg: OpsAgentConfig): ResolvedSignalAccount[] {
