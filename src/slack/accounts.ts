...
export function listSlackAccountIds(cfg: OpsAgentConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultSlackAccountId(cfg: OpsAgentConfig): string {
...
function mergeSlackAccountConfig(cfg: OpsAgentConfig, accountId: string): SlackAccountConfig {
...
export function listEnabledSlackAccounts(cfg: OpsAgentConfig): ResolvedSlackAccount[] {
