...
const spawnGatewayInstance = async (name: string): Promise<GatewayInstance> => {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `opsagent-e2e-${name}-`));
  const configDir = path.join(homeDir, ".opsagent");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          OPSAGENT_SKIP_CHANNELS: "1",
          OPSAGENT_SKIP_BROWSER_CONTROL_SERVER: "1",
          OPSAGENT_SKIP_CANVAS_HOST: "1",
        },
...
const waitForNodeStatus = async (inst: GatewayInstance, nodeId: string, timeoutMs = 10_000) => {
  ...
  while (Date.now() < deadline) {
    const list = (await runCliJson(
      ...
      {
        OPSAGENT_GATEWAY_TOKEN: inst.gatewayToken,
        OPSAGENT_GATEWAY_PASSWORD: "",
      },
    )) as NodeListPayload;
    ...
    async () => {
      ...
      const [healthA, healthB] = (await Promise.all([
        runCliJson(["health", "--json", "--timeout", "10000"], {
          OPSAGENT_GATEWAY_PORT: String(gwA.port),
          OPSAGENT_GATEWAY_TOKEN: gwA.gatewayToken,
          OPSAGENT_GATEWAY_PASSWORD: "",
        }),
        runCliJson(["health", "--json", "--timeout", "10000"], {
          OPSAGENT_GATEWAY_PORT: String(gwB.port),
          OPSAGENT_GATEWAY_TOKEN: gwB.gatewayToken,
          OPSAGENT_GATEWAY_PASSWORD: "",
        }),
