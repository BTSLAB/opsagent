...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("OPSAGENT_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  process.env.OPSAGENT_STATE_DIR = path.join(base, ".opsagent");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "opsagent-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".opsagent", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
