...
import type { OpsAgentApp } from "./app";
...
export async function refreshActiveTab(host: SettingsHost) {
  ...
  if (host.tab === "instances") await loadPresence(host as unknown as OpsAgentApp);
  if (host.tab === "sessions") await loadSessions(host as unknown as OpsAgentApp);
  if (host.tab === "cron") await loadCron(host);
  if (host.tab === "skills") await loadSkills(host as unknown as OpsAgentApp);
  if (host.tab === "nodes") {
    await loadNodes(host as unknown as OpsAgentApp);
    await loadDevices(host as unknown as OpsAgentApp);
    await loadConfig(host as unknown as OpsAgentApp);
    await loadExecApprovals(host as unknown as OpsAgentApp);
  }
  if (host.tab === "chat") {
  ...
  if (host.tab === "config") {
    await loadConfigSchema(host as unknown as OpsAgentApp);
    await loadConfig(host as unknown as OpsAgentApp);
  }
  if (host.tab === "debug") {
    await loadDebug(host as unknown as OpsAgentApp);
    host.eventLog = host.eventLogBuffer;
  }
  if (host.tab === "logs") {
    host.logsAtBottom = true;
    await loadLogs(host as unknown as OpsAgentApp, { reset: true });
    scheduleLogsScroll(
...
export function inferBasePath() {
  ...
  const configured = window.__OPSAGENT_CONTROL_UI_BASE_PATH__;
...
export async function loadOverview(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as OpsAgentApp, false),
    loadPresence(host as unknown as OpsAgentApp),
    loadSessions(host as unknown as OpsAgentApp),
    loadCronStatus(host as unknown as OpsAgentApp),
    loadDebug(host as unknown as OpsAgentApp),
  ]);
}

export async function loadChannelsTab(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as OpsAgentApp, true),
    loadConfigSchema(host as unknown as OpsAgentApp),
    loadConfig(host as unknown as OpsAgentApp),
  ]);
}
...
export async function loadCron(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as OpsAgentApp, false),
    loadCronStatus(host as unknown as OpsAgentApp),
    loadCronJobs(host as unknown as OpsAgentApp),
  ]);
}
